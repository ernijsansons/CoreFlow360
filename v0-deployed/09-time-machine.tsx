  ...
  return (
    <group ref={particlesRef}>
      {[...Array(20)].map((_, i) => (
        <mesh
          key={i}
          position={[(Math.random() - 0.5) * 10, 2 + Math.random() * 3, (Math.random() - 0.5) * 10]}
        >
  ...
  return (
    <group ref={streamRef}>
      {[...Array(6)].map((_, i) => (
        <mesh
          key={i}
          position={[Math.cos((i * Math.PI) / 3) * 4, 3, Math.sin((i * Math.PI) / 3) * 4]}
        >
  ...
  return (
    <group ref={assistantsRef}>
      {[...Array(4)].map((_, i) => (
        <mesh
          key={i}
          position={[Math.cos((i * Math.PI) / 2) * 6, 1, Math.sin((i * Math.PI) / 2) * 6]}
        >
  ...
  return (
    <group ref={sceneRef}>
      ...
      {[...Array(8)].map((_, i) => (
        <mesh
          key={i}
          position={[-2 + (i % 4) * 1.3, 0.5 + Math.random() * 0.5, -1 + Math.floor(i / 4) * 2]}
        >
      ...
      {[...Array(5)].map((_, i) => (
        <CoffeeCup key={i} position={[-2.5 + i * 1.2, 0.3, 1]} empty={Math.random() > 0.3} />
      ))}

      <StressParticles />

      <OwnerFigure position={[0, 2, 3]} state="stressed" animation="typing" />
    </group>
  )
}
  ...
  return (
    <group ref={cockpitRef} position={[0, 0, 5]}>
      ...
      <group position={[0, -1, 2]}>
        ...
        <StatusDisplay position={[2, 0, 0]} label="CONSCIOUSNESS" value="READY" color="#00ffff" />
      </group>

      {[...Array(8)].map((_, i) => (
        <pointLight
          key={i}
          position={[Math.cos((i * Math.PI) / 4) * 2.5, -0.5, Math.sin((i * Math.PI) / 4) * 2.5]}
          color="#00ffff"
...
const TimeStreamVisualization: React.FC<TimeStreamVisualizationProps> = ({
  progress,
  destination,
}) => {
  const streamRef = useRef<THREE.Group>(null)
  ...
  return (
    <group ref={streamRef}>
      <mesh>
        ...
        <meshBasicMaterial color="#4a90e2" transparent opacity={0.3} side={THREE.DoubleSide} />
      </mesh>
      ...
      {[...Array(200)].map((_, i) => (
        <mesh
          ...
          position={[(Math.random() - 0.5) * 20, (Math.random() - 0.5) * 20, -50 + (i / 200) * 100]}
        >
          <sphereGeometry args={[0.1]} />
          <meshBasicMaterial color={new THREE.Color().setHSL(destination * 0.1, 1, 0.5)} />
        </mesh>
      ))}
...
const FutureOfficeScene: React.FC<FutureOfficeSceneProps> = ({ scenario }) => {
  ...
  return (
    <group ref={sceneRef}>
      ...
      <Computer position={[0, 0.5, 0]} screenContent="dashboard" glowing={true} />

      <OwnerFigure position={[-3, 2, 2]} state="peaceful" animation="reading" />
...
const TimeMachineUI: React.FC<TimeMachineUIProps> = ({
  currentState,
  timeDestination,
  scenario,
  onReset,
  ...
  return (
    <div className="pointer-events-none fixed inset-0">
      <div className="absolute top-10 left-1/2 -translate-x-1/2 transform text-center">
        <h1 className="mb-4 bg-gradient-to-r from-violet-400 to-cyan-400 bg-clip-text text-6xl font-thin text-transparent text-white">
          Business Time Machine
        </h1>
        <p className="text-xl text-gray-400">Witness your future self living the dream</p>
      </div>
      ...
      {currentState === 'future' && scenario && (
        <>
          <div className="absolute top-10 right-10 max-w-md rounded-lg border border-emerald-500/30 bg-white/10 p-6 backdrop-blur-xl">
            <h3 className="mb-4 text-2xl font-bold">Future Scenario</h3>
