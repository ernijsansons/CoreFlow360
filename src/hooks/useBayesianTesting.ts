...
import {
  ...
  type BayesianResult,
  type BayesianConfig,
} from '@/lib/testing/bayesian-analysis'
...
export function useBayesianTesting(): UseBayesianTestingReturn {
  ...
  const createTest = useCallback(
    async (testData: Omit<ABTest, 'id' | 'status' | 'startDate'>): Promise<string> => {
  ...
  const updateTest = useCallback(
    async (testId: string, updates: Partial<ABTest>) => {
  ...
  const recordVisitor = useCallback(
    async (testId: string, variantId: string, userId: string) => {
  ...
  const analyzeTest = useCallback(
    async (testId: string): Promise<BayesianResult> => {
      const test = tests.find((t) => t.id === testId)
  ...
  const getTestRecommendation = useCallback(
    async (testId: string) => {
      ...
      const bestVariant = Object.entries(bayesianAnalysis.probabilityToBeatControl).reduce(
      ...
      switch (bayesianAnalysis.recommendedAction) {
        case 'continue':
          reasoning =
            `Test needs more data. Current sample size: ${analysis.results.reduce((sum, r) => sum + r.visitors, 0)} visitors. ` +
            `Continue until statistical significance is reached or maximum sample size is met.`
          break
        case 'stop_winner':
          reasoning =
            `Statistical significance reached with ${(bayesianAnalysis.confidence * 100).toFixed(1)}% confidence. ` +
            `Variant ${winningVariant} shows clear improvement over control.`
          break
        case 'stop_inconclusive':
          reasoning =
            `Maximum sample size reached but no clear winner emerged. ` +
