...
export class PerformanceTracker extends EventEmitter {
  ...
  startTracking(operation: string, options: TrackingOptions = {}): string {
    ...
    const startMemory =
      options.includeMemoryUsage !== false ? process.memoryUsage() : ({} as NodeJS.MemoryUsage)
    ...
    for (const [pattern, threshold] of this.thresholds.entries()) {
      if (this.matchesPattern(metric.operation, pattern)) {
        ...
        if (metric.duration > threshold.maxDuration) {
          violations.push(
            `Duration ${metric.duration.toFixed(2)}ms exceeds threshold ${threshold.maxDuration}ms`
          )
        }

        if (metric.memoryUsage.delta.heapUsed > threshold.maxMemoryDelta) {
          violations.push(
            `Memory delta ${(metric.memoryUsage.delta.heapUsed / 1024 / 1024).toFixed(2)}MB exceeds threshold ${(threshold.maxMemoryDelta / 1024 / 1024).toFixed(2)}MB`
          )
        }

        if (
          threshold.maxCpuDelta &&
          metric.cpuUsage?.delta.user &&
          metric.cpuUsage.delta.user > threshold.maxCpuDelta
        ) {
          violations.push(
            `CPU delta ${metric.cpuUsage.delta.user}μs exceeds threshold ${threshold.maxCpuDelta}μs`
          )
        }
  ...
  private toPrometheusFormat(): string {
    ...
    for (const [operation, stat] of Object.entries(stats)) {
      ...
      lines.push(
        `operation_duration_seconds{operation="${cleanOperation}"} ${stat.averageDuration / 1000}`
      )
