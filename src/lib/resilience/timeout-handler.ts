 ...
 */
function createRetryableError(attempt: number, maxAttempts: number, lastError: Error): RetryableError {
  const error = new Error(`Operation failed after ${attempt}/${maxAttempts} attempts: ${lastError.message}`) as RetryableError
...
export async function withRetry<T>(
  ...
  for (let attempt = 1; attempt <= config.retries + 1; attempt++) {
    try {
      ...
      console.warn(`Retrying operation '${operationName}' (attempt ${attempt}/${config.retries + 1}) after ${delay}ms:`, lastError?.message)
