...
export function createIDURARService(): IDURARService {
  return {
    async customerIntelligence(customerId: string) {
    ...
    async inventoryPrediction(productId: string, horizon: number) {
      ...
      for (let i = 0; i < horizon; i++) {
        ...
        const quantity = Math.round(
          baselineDemand * seasonalFactor * trend * randomVariation
        )
      ...
      const demandStdDev = Math.sqrt(
        demandForecast.reduce((sum, f) => sum + Math.pow(f.quantity - averageDemand, 2), 0) / horizon
      )
    ...
    async supplyChainOptimization(network: any) {
      ...
      const optimizedRoutes = routes.map((route: any) => ({
        ...route,
        optimized: true,
      ...
      const originalCost = routes.reduce((sum: number, r: any) => sum + r.cost, 0)
      const optimizedCost = optimizedRoutes.reduce((sum: number, r: any) => sum + r.newCost, 0)
      const costSavings = ((originalCost - optimizedCost) / originalCost) * 100
      
      const originalTime = routes.reduce((sum: number, r: any) => sum + r.time, 0)
      const optimizedTime = optimizedRoutes.reduce((sum: number, r: any) => sum + r.newTime, 0)
      const timeReduction = ((originalTime - optimizedTime) / originalTime) * 100
      
      const bottlenecks = locations
        .filter((loc: any) => loc.capacity && loc.demand > loc.capacity * 0.8)
        .map((loc: any) => ({
          location: loc.name,
