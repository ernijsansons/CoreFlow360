...
export function createERPNextService(): ERPNextService {
  return {
    async processAutomation(workflow: string, data: any) {
    ...
    async documentAnalysis(document: any) {
      ...
      switch (documentType) {
        ...
        case 'contract':
          ...
          extractedData.endDate = new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]
    ...
    async complianceCheck(entity: string, regulations: string[]) {
      ...
      regulations.forEach(regulation => {
        ...
        if (!isCompliant) {
          ...
          recommendations.push(`Review and update ${regulation} compliance procedures`)
          
          if (severity > 0.7) {
            requiredActions.push({
              action: `Immediate ${regulation} compliance audit required`,
      ...
      const complianceStatus = violations.length === 0 ? 'compliant' :
                              violations.some(v => v.severity === 3) ? 'non_compliant' : 'partial'
