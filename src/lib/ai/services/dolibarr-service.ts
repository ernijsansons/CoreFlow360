...
export function createDolibarrService(): DolibarrService {
  return {
    async contractAnalysis(contract: any) {
    ...
    async priceOptimization(product: any, market: any) {
      ...
      const avgCompetitorPrice = competition.reduce((a: number, b: number) => a + b, 0) / competition.length
