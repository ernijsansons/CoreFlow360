...
export class AIConversationEngine {
  ...
  async initializeConversation(websocket: WebSocket): Promise<void> {
    ...
    console.log(`🤖 AI Conversation initialized for call ${this.context.callSid}`)
    ...
    websocket.on('close', () => {
      console.log(`🔚 Conversation ended for call ${this.context.callSid}`)
   ...
   */
  private async handleIncomingAudio(data: any): Promise<void> {
    try {
      ...
      if (message.event === 'media' && message.media.payload) {
        ...
        if (this.audioBuffer.length >= 32 && !this.isProcessing) { // ~2 seconds at 16kHz
          await this.processAudioBuffer()
        }
      } else if (message.event === 'start') {
        console.log(`🎤 Audio stream started for call ${this.context.callSid}`)
      }
  ...
  private async transcribeAudio(audioBuffer: Buffer): Promise<string> {
    try {
      ...
      const transcription = await openai.audio.transcriptions.create({
        ...
        model: 'whisper-1',
        language: 'en',
        prompt: 'This is a business phone call about services. The customer may discuss HVAC, auto repair, insurance, or other professional services.'
      })
   ...
   */
  private buildChainOfThoughtPrompt(customerInput: string, script: any): {
    systemPrompt: string
    userPrompt: string
  } {
    const systemPrompt = `
...
`
    ...
    return expertise[this.context.industry] || `
...
`
  ...
  private updateConversationContext(aiResponse: AIResponse): void {
    ...
    if (aiResponse.chainOfThought.length > 0) {
      console.log(`🧠 Chain of Thought for call ${this.context.callSid}:`)
      aiResponse.chainOfThought.forEach((step, index) => {
        console.log(`  ${index + 1}. ${step.step}: ${step.reasoning} -> ${step.decision} (${step.confidence})`)
      })
    }
  }
  ...
  private async sendAIResponse(aiResponse: AIResponse): Promise<void> {
    try {
      ...
      if (aiResponse.shouldHandoffToHuman || aiResponse.confidence < ConversationConfidence.MEDIUM) {
  ...
  private async handleHumanHandoff(): Promise<void> {
    console.log(`👨‍💼 Initiating human handoff for call ${this.context.callSid}`)
    
    // Update state
    this.context.conversationState = ConversationState.HUMAN_HANDOFF
    
    // Send handoff message
    const handoffMessage = "Let me connect you with one of our specialists who can better assist you. Please hold for just a moment."
  ...
  private async handleAppointmentBooking(): Promise<void> {
    console.log(`📅 Initiating appointment booking for call ${this.context.callSid}`)
    ...
    try {
      ...
      if (appointment) {
        ...
        const unavailableMessage = "I apologize, but I don't have any available slots that match your preferences right now. Let me connect you with our scheduling team to find the best time for you."
      ...
      const errorMessage = "I'm having trouble accessing our scheduling system right now. Let me connect you with someone who can help schedule your appointment directly."
  ...
  private async sendFallbackResponse(): Promise<void> {
    const fallbackMessage = "I apologize, I didn't catch that clearly. Could you please repeat what you just said?"
  ...
  private async finalizeConversation(): Promise<void> {
    try {
      ...
      console.log(`✅ Conversation finalized for call ${this.context.callSid}`)
      console.log(`   Final score: ${this.context.qualificationScore}/10`)
      console.log(`   Outcome: ${this.determineCallOutcome()}`)
