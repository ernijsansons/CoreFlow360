...
export class SmartCache {
  private refreshQueue: Map<string, Promise<unknown>> = new Map()
  ...
  async get<T>(
    ...
    try {
      ...
      if (cached) {
        ...
        const isExpired =
          now.getTime() - cached.expiresAt.getTime() > configWithDefaults.staleTtl * 1000
  ...
  private async setData<T>(cacheKey: string, data: T, config: CacheConfig): Promise<void> {
    const now = new Date()
  ...
  private async fetchWithDeduplication<T>(cacheKey: string, fetcher: () => Promise<T>): Promise<T> {
