...
import { env as environment } from '@/lib/config/environment'
...
class RedisRateLimiter {
  ...
  constructor() {
    if (environment.REDIS_HOST && environment.REDIS_HOST !== 'localhost') {
...
export class RateLimiter {
  ...
  private useRedis = !!(environment.REDIS_HOST && environment.REDIS_HOST !== 'localhost')
...
export async function applyRateLimit(
  ...
  const rateLimitConfig = typeof configName === 'string' ? RateLimitConfigs[configName] : configName
