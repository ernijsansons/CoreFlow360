...
class CostMonitoringSocket {
  ...
  private async handleClientMessage(clientId: string, message: unknown, tenantId: string) {
    try {
  ...
  private async gatherTenantCostMetrics(_tenantId: string): Promise<CostMetric[]> {
    ...
    return mockMetrics.map((metric) => ({
      ...
      utilizationRate: Math.max(
        0.1,
        Math.min(1.0, metric.utilizationRate + (Math.random() - 0.5) * 0.2)
      ),
    }))
  }
  ...
  private async runAnomalyDetection(_metrics: CostMetric[], _tenantId: string): Promise<unknown[]> {
    const anomalies = []
    ...
    if (highCostResources.length > 0) {
      anomalies.push({
        ...
        estimatedImpact: highCostResources.reduce(
          (sum, r) => sum + (r.projectedCost - r.currentCost),
          0
        ),
        recommendations: [
  ...
  private async subscribeToCostMetrics(
    clientId: string,
