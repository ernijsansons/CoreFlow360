    ...
    datasources: {
      db: {
        url:
          ...
          `?connection_limit=${poolConfig.connection_limit}&pool_timeout=${poolConfig.pool_timeout}`,
      },
    },
  })
...
if (process.env.NODE_ENV === 'production') {
  ...
  if (!(global as unknown).prisma) {
    ;(global as unknown).prisma = createPrismaClient()
  }
  ...
  query: {
    ...
    $allModels: {
      async findMany({ model, operation, args, query }) {
        ...
        if (args.where && !args.where.tenantId && (global as unknown).currentTenantId) {
          args.where.tenantId = (global as unknown).currentTenantId
        }
      ...
      async create({ model, operation, args, query }) {
        ...
        if (args.data && !args.data.tenantId && (global as unknown).currentTenantId) {
          args.data.tenantId = (global as unknown).currentTenantId
        }
  ...
  model: {
    $allModels: {
      ...
      async softDelete<T>(this: T, where: unknown) {
        ...
        // @ts-ignore
        return (context as unknown).update({
          where,
          data: {
            deletedAt: new Date(),
          },
      ...
      async findManyActive<T>(this: T, args?: unknown) {
        const context = Prisma.getExtensionContext(this)
        // @ts-ignore
        return (context as unknown).findMany({
          ...args,
          where: {
            ...args?.where,
            deletedAt: null,
      ...
      async countActive<T>(this: T, args?: unknown) {
        const context = Prisma.getExtensionContext(this)
        // @ts-ignore
        return (context as unknown).count({
          ...args,
          where: {
            ...args?.where,
            deletedAt: null,
...
export const dbUtils = {
   ...
   * Batch operations
   */
  async batchCreate<T>(model: unknown, data: T[], batchSize: number = 100): Promise<number> {
    let created = 0
  ...
  async getPoolStats() {
    try {
      const [poolStats] = await prisma.$queryRaw<unknown[]>`
        SELECT 
...
export function setCurrentTenant(tenantId: string | null) {
  ;(global as unknown).currentTenantId = tenantId
}
...
export function clearCurrentTenant() {
  delete (global as unknown).currentTenantId
}
