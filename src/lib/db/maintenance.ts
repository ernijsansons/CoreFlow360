...
export const dbMaintenance = {
  ...
  async cleanup(options: {
    ...
    for (const task of cleanupTasks) {
      ...
      try {
        ...
        console.log(`✅ ${task.name}: ${count} records ${dryRun ? 'would be' : ''} deleted`)
      } catch (error) {
        console.error(`❌ Failed to cleanup ${task.name}:`, error)
  ...
  async healthCheck(): Promise<{
    ...
    // Index usage check
    try {
      const indexStats = await db.$queryRaw<any[]>`
        SELECT 
          schemaname,
          tablename,
          attname,
    ...
    // Slow queries check
    try {
      const slowQueries = await db.$queryRaw<any[]>`
        SELECT 
          query,
          calls,
          total_time,
      ...
      checks.push({
        ...
        message: slowQueries.length > 0 ? `${slowQueries.length} slow queries detected` : 'No slow queries'
      })
  ...
  async generateStatsReport() {
    console.log('📊 Generating database statistics report...')
