...
class WorkerManager {
   ...
   * Handle worker errors
   */
  private async handleWorkerError(_name: string, _error: Error) {
    ...
    this.monitorInterval = setInterval(async () => {
      try {
        ...
        for (const [name, workerStats] of Object.entries(stats.workers)) {
          const worker = workerStats as unknown
          if (worker.status === 'stopped') {
          }

          if (worker.errorRate > 0.1) {
            // More than 10% error rate
            console.warn(
              `⚠️ High error rate for ${name} worker: ${(worker.errorRate * 100).toFixed(1)}%`
            )
          }
        }
        ...
        for (const [queue, counts] of Object.entries(stats.queues)) {
          const queueCounts = counts as unknown
