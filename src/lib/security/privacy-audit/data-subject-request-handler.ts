...
export class DataSubjectRequestHandler extends EventEmitter {
  ...
  async processAccessRequest(requestId: string): Promise<boolean> {
    ...
    try {
      ...
      request.processingNotes.push(
        `Started access request processing at ${new Date().toISOString()}`
      )
  ...
  async processExportRequest(requestId: string): Promise<boolean> {
    ...
    try {
      request.status = 'processing'
      request.processingNotes.push(
        `Started export request processing at ${new Date().toISOString()}`
      )
  ...
  async processDeletionRequest(requestId: string): Promise<boolean> {
    ...
    try {
      request.status = 'processing'
      request.processingNotes.push(
        `Started deletion request processing at ${new Date().toISOString()}`
      )
      ...
      if (failedDeletions.length > 0) {
        request.processingNotes.push(
          `Some data could not be deleted: ${failedDeletions.map((f) => f.system).join(', ')}`
        )
      }
  ...
  async processCorrectionRequest(
    ...
    try {
      request.status = 'processing'
      request.processingNotes.push(
        `Started correction request processing at ${new Date().toISOString()}`
      )
  ...
  async generateComplianceReport(timeRange: { start: Date; end: Date }): Promise<{
    ...
    const requests = Array.from(this.requests.values()).filter(
      (req) => req.requestDate >= timeRange.start && req.requestDate <= timeRange.end
    )
    ...
    const jurisdictionBreakdown = requests.reduce(
      (acc, req) => {
    ...
    const typeBreakdown = requests.reduce(
      (acc, req) => {
  ...
  private async checkOverdueRequests(): Promise<void> {
    const now = new Date()
    const overdueRequests = Array.from(this.requests.values()).filter(
      (req) => req.dueDate < now && !['completed', 'cancelled', 'rejected'].includes(req.status)
    )
  ...
  private async collectUserData(_userId: string, categories?: string[]): Promise<unknown> {
  ...
  private async deleteUserData(
    ...
    return systems.map((system) => ({
      ...
      timestamp: new Date(),
      error:
        exemptions.includes('legal_obligation') && system === 'backup_system'
