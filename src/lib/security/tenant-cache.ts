...
export class TenantSecureCache {
  ...
  constructor() {
    ...
    this.cacheSecret =
      process.env.CACHE_ENCRYPTION_SECRET || 'default-cache-secret-change-in-production'
  }
   ...
   */
  private generateSecureCacheKey(key: string, context: TenantCacheContext): string {
   ...
   */
  private encryptCacheValue(value: unknown): string {
   ...
   * Decrypt cache value
   */
  private decryptCacheValue(encrypted: string): unknown | null {
    try {
   ...
   */
  async get<T = unknown>(key: string, context: TenantCacheContext): Promise<T | null> {
    try {
   ...
   */
  async delete(key: string, context: TenantCacheContext): Promise<boolean> {
    try {
      const secureKey = this.generateSecureCacheKey(key, context)
 ...
 */
export function withTenantCache<T extends (...args: unknown[]) => unknown>(
  handler: T,
  options: {
    cacheKey: string
    ttl?: number
  ...
  return (async (...args: unknown[]) => {
    ...
    if (request?.method === 'GET' && result && !result.error) {
      await tenantCache.set(options.cacheKey, result, cacheContext, { ttl: options.ttl })
    }
...
export const TenantCacheUtils = {
   ...
   */
  listCacheKey(entityType: string, filters?: Record<string, unknown>): string {
  ...
  async invalidateEntityCache(
    ...
    if (entityId) {
      ...
      await tenantCache.delete(TenantCacheUtils.entityCacheKey(entityType, entityId), cacheContext)
    }

    // Invalidate list caches for this entity type
    await tenantCache.delete(TenantCacheUtils.listCacheKey(entityType), cacheContext)
  },
}
