...
export class RequestSigner {
  ...
  constructor(config: Partial<SigningConfig> = {}) {
    ...
    this.config = {
      secretKey:
        process.env.API_SIGNING_SECRET || (isBuildTime ? 'build-time-placeholder-secret' : ''),
      algorithm: 'sha256',
   ...
   */
  async validateRequest(req: NextRequest, body?: string): Promise<ValidationResult> {
    try {
  ...
  private buildPayload(params: {
    ...
    const components = [params.method, params.url, params.timestamp.toString(), params.nonce]
    ...
    if (params.headers) {
      const sortedHeaders = Object.keys(params.headers)
        .filter(
          (key) =>
            key.toLowerCase().startsWith('x-coreflow-') || key.toLowerCase() === 'content-type'
        )
        .sort()
  ...
  private extractRelevantHeaders(req: NextRequest): Record<string, string> {
    ...
    req.headers.forEach((value, key) => {
      if (key.toLowerCase().startsWith('x-coreflow-') || key.toLowerCase() === 'content-type') {
        headers[key] = value
      }
    })
...
export function withRequestSigning(options: Partial<SigningConfig> = {}) {
  ...
  return async function signatureMiddleware(
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development' && process.env.SKIP_REQUEST_SIGNING === 'true') {
        return handler(req)
      }
