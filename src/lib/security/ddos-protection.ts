...
export class DDoSProtection {
  ...
  private static checkGeoBlocking(country?: string): DDoSProtectionResult {
    ...
    if (DDOS_CONFIG.geoBlocking.allowedCountries.length > 0 &&
  ...
  private static checkTrafficPatterns(
    ...
    const score = Math.min(40, (uniquePaths.size / DDOS_CONFIG.trafficPatterns.maxUniquePathsPerIP) * 40)
  ...
  private static checkUserAgent(userAgent: string): DDoSProtectionResult {
    ...
    const isSuspicious = DDOS_CONFIG.trafficPatterns.suspiciousUserAgents.some(
  ...
  private static extractCloudflareHeaders(request: NextRequest): CloudflareHeaders {
    ...
    const headerNames = [
      'cf-ray', 'cf-connecting-ip', 'cf-ipcountry',
      'cf-threat-score', 'cf-visitor', 'cf-request-id', 'cf-challenge'
    ]
  ...
  static generateChallengeResponse(
    ...
    const html = action === 'challenge' 
      ? this.getJavascriptChallengeHTML()
      : this.getCaptchaChallengeHTML()
