...
export class SecurityOperationsManager {
  ...
  private async detectSuspiciousActivity(
    ...
    const recentEntries = this.auditLog.filter(
      (entry) =>
        entry.userId === context.userId && Date.now() - new Date(entry.timestamp).getTime() < 300000 // 5 minutes
    )
  ...
  private async logAuditEntry(entry: AuditLogEntry): Promise<void> {
    ...
    if (
      entry.hash !==
      this.generateAuditHash(entry.id, entry.securityContext, entry.operation, entry.success)
    ) {
  ...
  getSecurityMetrics(): {
    ...
    const avgRisk =
      total > 0 ? this.auditLog.reduce((sum, entry) => sum + entry.riskScore, 0) / total : 0
    const recentHighRisk = this.auditLog.filter(
      (entry) => entry.riskScore > 70 && Date.now() - new Date(entry.timestamp).getTime() < 3600000 // 1 hour
    ).length
