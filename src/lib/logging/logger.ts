...
class CoreFlowLogger {
   ...
   */
  private sendToMonitoring(alertType: string, data: unknown) {
...
export type { LogContext, SecurityLogData, AuditLogData, PerformanceLogData, BusinessLogData }
...
export function createRequestLogger() {
  return (req: unknown, res: unknown, next: unknown) => {
    const requestId =
      req.headers['x-request-id'] || req.id || Math.random().toString(36).substring(7)
