...
export class BundleEventBus extends EventEmitter {
  ...
  async publishEvent(event: Omit<BundleEvent, 'id' | 'metadata.timestamp'>): Promise<void> {
    ...
    try {
      ...
      const publishPromises = targetBundles.map((bundleId) =>
        this.redis.publish(`bundle:${bundleId}:${fullEvent.type}`, JSON.stringify(processedEvent))
      )
  ...
  private async resolveTargetBundles(event: BundleEvent): Promise<BundleId[]> {
    if (event.target?.broadcast) {
      ...
      return Array.from(this.activeSubscriptions.get(event.metadata.tenantId) || new Set())
    }
  ...
  async subscribeBundle(
    ...
    try {
      ...
      console.log(`Bundle ${bundleId} subscribed to events: ${eventTypes.join(', ')}`)
    } catch (error) {
      console.error(`Failed to subscribe bundle ${bundleId}:`, error)
      throw error
    }
  }
  ...
  async unsubscribeBundle(tenantId: string, bundleId: BundleId): Promise<void> {
    try {
      ...
      console.log(`Bundle ${bundleId} unsubscribed from events`)
    } catch (error) {
      console.error(`Failed to unsubscribe bundle ${bundleId}:`, error)
      throw error
    }
  }
  ...
  private collectMetrics(): void {
    ...
    const recentMetrics = Array.from(this.metrics.values()).filter(
      (m) => now - m.timestamp.getTime() < 60000
    ) // Last minute
  ...
  async getMetrics(): Promise<{
    ...
    const avgProcessingTime =
      totalEvents > 0 ? metrics.reduce((sum, m) => sum + m.processingTime, 0) / totalEvents : 0
