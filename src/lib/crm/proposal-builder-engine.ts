...
export class ProposalBuilderEngine {
   ...
   */
  async calculateROI(_request: ProposalRequest, _analysis: unknown): Promise<ROICalculation> {
    ...
    const productivityGains = await this.calculateProductivityGains(
      request.client,
      request.products
    )
    ...
    const fiveYearROI =
      ((cashFlows.reduce((sum, cf) => sum + cf, 0) - totalInvestment) / totalInvestment) * 100
    ...
    const sensitivity = this.performSensitivityAnalysis(totalInvestment, annualReturn, assumptions)
  ...
  private assessDigitalMaturity(client: ClientInfo): string {
    ...
    const hasBasicTech = client.currentChallenges.some(
      (c) =>
    ...
    const hasAdvancedGoals = client.goals.some(
      (g) =>
  ...
  private assessUrgency(goals: BusinessGoal[], timeline: string): 'LOW' | 'MEDIUM' | 'HIGH' {
    ...
    const urgentTimeline =
      timeline.toLowerCase().includes('immediate') || timeline.includes('30 days')
  ...
  private identifyDifferentiators(criteria: string[]): string[] {
    ...
    if (
      criteria.some((c) => c.toLowerCase().includes('price') || c.toLowerCase().includes('cost'))
    ) {
      differentiators.push('50-70% lower TCO than enterprise competitors')
    }
    if (
      criteria.some((c) => c.toLowerCase().includes('ease') || c.toLowerCase().includes('simple'))
    ) {
      differentiators.push('No-code automation and intuitive interface')
    }
    if (
      criteria.some(
        (c) => c.toLowerCase().includes('ai') || c.toLowerCase().includes('intelligence')
      )
    ) {
  ...
  private identifyWinThemes(_client: ClientInfo, _opportunity: OpportunityInfo): string[] {
    const themes = []
    ...
    if (
      client.goals.some(
        (g) => g.description.includes('innovation') || g.description.includes('transform')
      )
    ) {
  ...
  private async generateProblemStatement(client: ClientInfo): Promise<ProposalSection> {
    return {
      ...
      content: {
        ...
        summary:
          'These challenges represent not just operational inefficiencies, but missed opportunities for growth and competitive advantage.',
      },
      order: 3,
      required: true,
      customizable: true,
  ...
  private async generateSolutionSection(request: ProposalRequest): Promise<ProposalSection> {
    return {
      ...
      content: {
        overview:
          'CoreFlow360 provides a comprehensive, AI-powered platform that addresses your challenges through intelligent automation and data-driven insights.',
        approach: {
          phases: [
            'Discovery & Planning',
  ...
  private identifyBundles(products: ProductSelection[]): unknown[] {
  ...
  private async generateImplementationSection(
    products: ProductSelection[]
  ): Promise<ProposalSection> {
    ...
    return {
      ...
      content: {
        overview:
          'Our proven implementation methodology ensures rapid time-to-value with minimal disruption.',
  ...
  private async generateROISection(_request: ProposalRequest): Promise<ProposalSection> {
  ...
  private async generateTestimonialsSection(_client: ClientInfo): Promise<ProposalSection> {
    return {
      id: 'testimonials',
      type: 'TESTIMONIALS',
  ...
  private async generatePricingSection(products: ProductSelection[]): Promise<ProposalSection> {
    const pricing = {
      ...
      totalDiscounts: products.reduce(
        (sum, p) => sum + (p.pricing.discounts?.reduce((d, disc) => d + disc.value, 0) || 0),
        0
      ),
  ...
  private async calculateRevenueGains(
    client: ClientInfo,
    products: ProductSelection[]
  ): Promise<RevenueItem[]> {
  ...
  private async calculateProductivityGains(
    client: ClientInfo,
    products: ProductSelection[]
  ...
  private async findRelevantCaseStudies(client: ClientInfo): Promise<CaseStudy[]> {
    ...
    const allCaseStudies: CaseStudy[] = [
      ...
      },
      {
        ...
        results: [
          {
            metric: 'Compliance',
            improvement: '100% audit pass rate',
            timeframe: '6 months',
            verified: true,
          },
          {
    ...
    return allCaseStudies.sort((a, b) => b.similarityScore - a.similarityScore).slice(0, 3)
  }
  ...
  private async selectTestimonials(industry: string): Promise<Testimonial[]> {
    ...
    const testimonials: Testimonial[] = [
      {
        quote:
          'CoreFlow360 delivered on every promise. Our efficiency has skyrocketed, and the ROI was evident within months.',
        author: 'Sarah Chen',
      ...
      {
        quote:
          "The AI capabilities are game-changing. We're making decisions faster and with more confidence than ever before.",
      ...
      {
        quote:
          'Implementation was smooth, support is excellent, and the platform keeps getting better. Highly recommended.',
        author: 'Jennifer Wu',
        title: 'CEO',
  ...
  private async generatePDF(document: unknown): Promise<string> {
    // Convert HTML to PDF

    return `/proposals/pdf/${document.id}.pdf`
  }

  private async generateOnlineVersion(document: unknown): Promise<string> {
