...
export abstract class BaseExternalService {
  ...
  protected validateSecurity(context: SecurityContext): boolean {
    return (
      ...
      context.userId && 
      context.sessionId && 
      Array.isArray(context.bundleAccess)
    )
  }
  ...
  private updateMetrics(success: boolean, responseTime: number): void {
    ...
    if (success) {
      this.metrics.successRate = (this.metrics.successRate * (this.metrics.requestCount - 1) + 1) / this.metrics.requestCount
    } else {
      this.metrics.errorCount++
      this.metrics.successRate = (this.metrics.successRate * (this.metrics.requestCount - 1)) / this.metrics.requestCount
    }
  ...
  protected getAuthHeaders(context: SecurityContext): Record<string, string> {
    ...
    switch (this.config.authentication.type) {
      case 'api_key':
        ...
        headers[keyHeader] = process.env[`${this.config.service.toUpperCase()}_API_KEY`] || 'dev-key'
        break
  ...
  private generateJWT(context: SecurityContext): string {
    // In production, use proper JWT library with RS256
    return 'mock.jwt.token'
  }

  private getOAuth2Token(context: SecurityContext): string {
