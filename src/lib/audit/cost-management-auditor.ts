...
class CostManagementAuditor {
  ...
  async utilityOptimizer(tenantId: string): Promise<CostAuditResult> {
    ...
    const waste = utilizationMetrics
      ...
      .map((metric) => {
        const savings =
          metric.monthlyCost * (1 - metric.idealUtilization / metric.currentUtilization)
        totalPotentialSavings += savings
        recommendations.push(
          `Rightsize ${metric.resource}: ${Math.round(savings)}% cost reduction possible`
        )
        return {
    ...
    return {
      ...
      criticalIssues: waste
        .filter((w) => w.savings > 1000)
        .map((w) => `High waste in ${w.resource}`),
    }
  }
  ...
  async lockInAssessor(tenantId: string): Promise<CostAuditResult> {
    ...
    const migrations = vendors.filter(
      (v) => v.lockInLevel === 'HIGH' || v.lockInLevel === 'CRITICAL'
    )
    ...
    return {
      ...
      recommendations,
      potentialSavings: 0,
      criticalIssues: migrations
        .filter((m) => m.lockInLevel === 'CRITICAL')
        .map((m) => `Critical lock-in: ${m.vendor}`),
    }
  }
  ...
  async finOpsProcessor(tenantId: string): Promise<CostAuditResult> {
    ...
    const untaggedPercentage =
      finOpsMetrics.untaggedResources /
      (finOpsMetrics.taggedResources + finOpsMetrics.untaggedResources)
    ...
    if (untaggedPercentage > 0.2) {
      recommendations.push(
        `Tag ${finOpsMetrics.untaggedResources} untagged resources for cost allocation`
      )
    }
    if (Math.abs(finOpsMetrics.budgetVariance) > 0.1) {
      recommendations.push(
        `Budget variance of ${Math.round(finOpsMetrics.budgetVariance * 100)}% requires attention`
      )
    }
    if (finOpsMetrics.forecastAccuracy < 0.8) {
      recommendations.push(
        `Improve forecasting accuracy (currently ${Math.round(finOpsMetrics.forecastAccuracy * 100)}%)`
      )
    }
  ...
  async transferMinimizer(tenantId: string): Promise<CostAuditResult> {
    ...
    return {
      ...
      criticalIssues:
        transferCosts.currentEgressCosts > 10000 ? ['High data transfer costs detected'] : [],
    }
  }
  ...
  async tcoAggregator(tenantId: string): Promise<CostAuditResult> {
    ...
    const benchmarkVariance =
      ((tcoAnalysis.totalCost - tcoAnalysis.industryBenchmark) / tcoAnalysis.industryBenchmark) *
      100
    ...
    return {
      ...
      recommendations,
      potentialSavings:
        benchmarkVariance > 0 ? tcoAnalysis.totalCost - tcoAnalysis.industryBenchmark : 0,
      criticalIssues: benchmarkVariance > 50 ? ['TCO significantly above industry benchmark'] : [],
    }
  }

  private async analyzeResourceUtilization(_tenantId: string): Promise<UtilizationMetrics[]> {
  ...
  private async assessVendorLockIn(_tenantId: string): Promise<VendorLockInAssessment[]> {
    return [
      {
        vendor: 'AWS',
        lockInLevel: 'MEDIUM',
  ...
  private async analyzeFinOpsMaturity(_tenantId: string): Promise<FinOpsMetrics> {
    return {
      budgetVariance: 0.15,
  ...
  private async analyzeDataTransferCosts(_tenantId: string): Promise<DataTransferCosts> {
    return {
  ...
  private async calculateTotalCostOfOwnership(_tenantId: string): Promise<TCOAnalysis> {
    return {
      totalCost: 250000,
      ...
      await prisma.aiActivity.create({
        data: {
          ...
          details: JSON.stringify({
            ...
            totalPotentialSavings: this.auditResults.reduce(
