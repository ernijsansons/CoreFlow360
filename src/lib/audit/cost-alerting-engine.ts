...
class CostAlertingEngine {
  ...
  async createAlert(
    alertData: Omit<CostAlert, 'alertId' | 'status' | 'escalationLevel'>
  ): Promise<CostAlert> {
  ...
  async acknowledgeAlert(
    ...
    this.addToHistory(
      ...
      'acknowledged',
      acknowledgedBy,
      `Alert acknowledged${notes ? `: ${notes}` : ''}`
    )
  ...
  async getAlertStatistics(
    ...
    const tenantAlerts = Array.from(this.activeAlerts.values()).filter(
    ...
    const autoRemediationRate =
      resolvedAlerts.length > 0 ? autoRemediatedAlerts.length / resolvedAlerts.length : 0
  ...
  private async sendEmailNotification(_recipient: Recipient, _alert: CostAlert, template?: string) {
  ...
  private async sendSlackNotification(config: Record<string, unknown>, alert: CostAlert) {
    // Slack notification implementation
    logger.info('Slack notification sent', { channel: config.channel, alertId: alert.alertId })
  }

  private async sendWebhookNotification(config: Record<string, unknown>, alert: CostAlert) {
    // Webhook notification implementation
    logger.info('Webhook notification sent', { url: config.url, alertId: alert.alertId })
  }

  private async sendPagerDutyNotification(config: Record<string, unknown>, alert: CostAlert) {
  ...
  private async sendAutoRemediationNotification(_alert: CostAlert, _result: unknown) {
    // Send auto-remediation notification
    logger.info('Auto-remediation notification sent', { alertId: alert.alertId })
  }

  private async sendExecutiveAlert(_alert: CostAlert, _config: Record<string, unknown>) {
    // Executive alert implementation
    logger.info('Executive alert sent', { alertId: alert.alertId })
  }

  // Resource action methods
  private async scaleDownResources(resources: string[], config: Record<string, unknown>) {
    // Scale down resource implementation
    logger.info('Resources scaled down', { resources, percentage: config.percentage })
  }

  private async pauseResources(_resources: string[], _config: Record<string, unknown>) {
    // Pause resource implementation
    logger.info('Resources paused', { resources })
  }

  private async createSupportTicket(_alert: CostAlert, _config: Record<string, unknown>) {
  ...
  private async getHistoricalAlerts(
    _tenantId: string,
    timeRange?: { start: Date; end: Date }
  ): Promise<CostAlert[]> {
    // Mock implementation
  ...
  private async getAutoRemediatedAlerts(
    _tenantId: string,
    timeRange?: { start: Date; end: Date }
  ): Promise<CostAlert[]> {
    // Mock implementation
