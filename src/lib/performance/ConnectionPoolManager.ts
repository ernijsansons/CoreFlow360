...
export class ConnectionPoolManager extends EventEmitter {
  ...
  async executeWithPool<T>(
    ...
    try {
      ...
      if (options?.retries && options.retries > 0) {
        return this.executeWithPool(tenantId, operation, {
          ...options,
  ...
  async autoScalePools(): Promise<void> {
    for (const [tenantId, metrics] of this.poolMetrics) {
      ...
      if ((utilizationRate > 0.8 || queuePressure || highLatency) && health.status !== 'critical') {
        ...
        shouldScale = true
        console.log(
          `📈 Scaling UP pool for tenant ${tenantId}: ${config.maxConnections} → ${newMaxConnections}`
        )
      }
      ...
      if (utilizationRate < 0.3 && !queuePressure && !highLatency) {
        ...
        if (newMaxConnections !== config.maxConnections) {
          shouldScale = true
          console.log(
            `📉 Scaling DOWN pool for tenant ${tenantId}: ${config.maxConnections} → ${newMaxConnections}`
          )
        }
      }
    ...
    const topPools = pools
      .map(([tenantId, metrics]) => {
        ...
        const utilization =
          metrics.totalConnections > 0 ? metrics.activeConnections / metrics.totalConnections : 0
        const errorRate =
          metrics.totalAcquires > 0 ? (metrics.totalErrors / metrics.totalAcquires) * 100 : 0
  ...
  // Private methods
  private async createPool(_tenantId: string, _config: PoolConfig): Promise<PrismaClient> {
