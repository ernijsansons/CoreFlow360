...
export class PerformanceTracker {
  ...
  finishCallTracking(callId: string): CallPerformanceData | null {
    ...
    // Calculate ROI
    const roi =
      callData.revenue && callData.cost ? (callData.revenue - callData.cost) / callData.cost : 0
  ...
  private async collectSystemMetrics(): Promise<SystemHealthMetrics> {
    ...
    const vapiAvailability =
      vapiCalls.length > 0
        ? 1 - vapiCalls.filter((c) => c.name === 'call_error').length / vapiCalls.length
        : 1

    const twilioAvailability =
      twilioCalls.length > 0
        ? 1 - twilioCalls.filter((c) => c.name === 'call_error').length / twilioCalls.length
        : 1
    ...
    const errorRate = totalCalls.length > 0 ? errors.length / totalCalls.length : 0

    const conversionRate = totalCalls.length > 0 ? appointments.length / totalCalls.length : 0
      ...
      costPerCall:
        totalCalls.length > 0
          ? recentMetrics
              ...
              .reduce((sum, m) => sum + m.value, 0) / totalCalls.length
          : 0,
      conversionRate,
  ...
  private checkAlertThresholds(metrics: SystemHealthMetrics): void {
    ...
    if (metrics.averageLatency > this.alertThresholds.latencyP99) {
      alerts.push(
        `High latency: ${metrics.averageLatency.toFixed(0)}ms > ${this.alertThresholds.latencyP99}ms`
      )
    }

    if (metrics.errorRate > this.alertThresholds.errorRate) {
      alerts.push(
        `High error rate: ${(metrics.errorRate * 100).toFixed(2)}% > ${(this.alertThresholds.errorRate * 100).toFixed(2)}%`
      )
    }

    if (metrics.vapiAvailability < this.alertThresholds.availability) {
      alerts.push(
        `Low Vapi availability: ${(metrics.vapiAvailability * 100).toFixed(3)}% < ${(this.alertThresholds.availability * 100).toFixed(3)}%`
      )
    }

    if (metrics.conversionRate < this.alertThresholds.conversionRate) {
      alerts.push(
        `Low conversion rate: ${(metrics.conversionRate * 100).toFixed(2)}% < ${(this.alertThresholds.conversionRate * 100).toFixed(2)}%`
      )
    }
  ...
  private aggregateMetrics(): void {
    ...
    const aggregates = recentMetrics.reduce(
      (acc, metric) => {
  ...
  public getAverageRevenue(tenantId?: string, timeRange?: { start: Date; end: Date }): number {
    ...
    return revenueMetrics.length > 0
      ? revenueMetrics.reduce((sum, m) => sum + m.value, 0) / revenueMetrics.length
      : 0
  }
