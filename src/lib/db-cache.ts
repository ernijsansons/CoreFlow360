...
export class DatabaseCache {
  ...
  async updateUser(userId: string, data: unknown, tenantId: string): Promise<User> {
    // Update in database
    ...
    await redis.set(cacheKey('user', userId), updatedUser, { tenantId, ttl: 300 })
  ...
  async updateTenantSubscription(tenantId: string, data: unknown): Promise<TenantSubscription> {
    // Update in database
    ...
    // Update cache with fresh data
    await redis.set(cacheKey('subscription', tenantId), updatedSubscription, { tenantId, ttl: 300 })
  ...
  async getTenantStats(tenantId: string) {
    return redis.cache(
      ...
      async () => {
        const [totalUsers, activeUsers, totalCustomers, activeDeals, totalRevenue, openProjects] =
          await Promise.all([
