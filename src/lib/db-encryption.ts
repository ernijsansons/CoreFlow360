 ...
 */
export const piiEncryptionMiddleware = async (params: any, next: any) => {
 ...
 */
function encryptPIIFields(data: any, fields: string[]): any {
 ...
 */
function decryptPIIFields(data: any, fields: string[]): any {
  if (!data || typeof data !== 'object') {
    return data
  }
 ...
 */
export async function migrateUnencryptedPIIData(prisma: any) {
  const batchSize = 100
  let processed = 0
  ...
  while (true) {
    ...
    console.log(`Processed ${customerSkip} customers...`)
  }
  ...
  while (true) {
    ...
    userSkip += batchSize
    console.log(`Processed ${userSkip} users...`)
  }
  
  console.log(`PII encryption migration completed. Processed ${processed} records.`)
}