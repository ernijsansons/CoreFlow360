...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('📊 Fetching observability analytics for tenant:', query.tenantId)
    ...
    if (query.includeMetrics) {
      response.metrics = {
        summary: {
          ...
          avgValue: dashboard.metrics.reduce((sum, m) => sum + m.value, 0) / dashboard.metrics.length || 0,
    ...
    if (query.includeLogs) {
      ...
      response.logs = {
        summary: {
          ...
          errorRate: (filteredLogs.filter(l => l.level === 'error').length / filteredLogs.length) * 100 || 0,
          warnRate: (filteredLogs.filter(l => l.level === 'warn').length / filteredLogs.length) * 100 || 0,
    ...
    if (query.includeTraces) {
      response.traces = {
        summary: {
          ...
          avgDuration: dashboard.traces.reduce((sum, t) => sum + (t.duration || 0), 0) / dashboard.traces.length || 0,
          successRate: (dashboard.traces.filter(t => t.status === 'success').length / dashboard.traces.length) * 100 || 0,
    ...
    console.error('❌ Failed to fetch observability analytics:', error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (action) {
      case 'metric':
        console.log('📊 Collecting custom metric')
      ...
      case 'trace':
        console.log('🔍 Creating trace span')
    ...
    console.error('❌ Failed to process observability action:', error)
    return NextResponse.json(
      {
        success: false,
...
// Helper functions
async function generateMetricTrends(metrics: any[]): Promise<any> {
...
function generateLogLevelDistribution(logs: any[]): any {
...
function generateTracePerformanceAnalysis(traces: any[]): any {
...
function generateAlertTrends(alerts: any[]): any {
...
function calculateOverallSystemHealth(dashboard: any): string {
  const errorRate = dashboard.logs.filter((l: any) => l.level === 'error').length / dashboard.logs.length || 0
  const criticalAlerts = dashboard.alerts.filter((a: any) => a.severity === 'CRITICAL').length
  const avgResponseTime = dashboard.traces.reduce((sum: number, t: any) => sum + (t.duration || 0), 0) / dashboard.traces.length || 0
...
function generateServiceHealthMap(dashboard: any): Record<string, any> {
  const services = new Set([
    ...dashboard.logs.map((l: any) => l.service),
    ...dashboard.traces.map((t: any) => t.service),
    ...dashboard.metrics.map((m: any) => m.source)
  ])
  
  const healthMap: Record<string, any> = {}
  
  services.forEach(service => {
    const serviceLogs = dashboard.logs.filter((l: any) => l.service === service)
    const serviceTraces = dashboard.traces.filter((t: any) => t.service === service)
    
    const errorRate = serviceLogs.filter((l: any) => l.level === 'error').length / serviceLogs.length || 0
    const avgDuration = serviceTraces.reduce((sum: number, t: any) => sum + (t.duration || 0), 0) / serviceTraces.length || 0
...
function generateHealthRecommendations(dashboard: any): string[] {
  ...
  if (avgResponseTime > 1000) {
    recommendations.push('Response times are elevated - consider implementing caching or optimizing slow operations')
  }
  
  const criticalAlerts = dashboard.alerts.filter((a: any) => a.severity === 'CRITICAL' && a.status === 'OPEN').length
  if (criticalAlerts > 0) {
...
async function generateBusinessTrends(businessMetrics: any[]): Promise<any> {
...
function generateHourlyTrends(data: any[]): any[] {
...
function generateHourlyAlertTrends(alerts: any[]): any[] {
...
function calculateMetricTrend(metrics: any[]): any {
...
function convertDashboardToCSV(dashboard: any): string {
  ...
  // Add metrics
  dashboard.metrics.forEach((metric: any) => {
