...
async function getHandler(request: NextRequest) {
  try {
    ...
    console.error('Failed to get SAML configurations:', error);
    ...
    return NextResponse.json(
      { error: 'Failed to get SAML configurations' },
      { status: 500 }
    );
  }
}
...
async function postHandler(request: NextRequest) {
  try {
    ...
    const config: SAMLConfig = {
      ...
      cert: data.certificate.replace(/-----BEGIN CERTIFICATE-----/g, '')
  ...
  try {
    ...
    if (updated.isActive) {
      const config: SAMLConfig = {
        ...
        cert: updated.certificate.replace(/-----BEGIN CERTIFICATE-----/g, '')
          .replace(/-----END CERTIFICATE-----/g, '')
          .replace(/\s/g, ''),
        callbackUrl: updated.callbackUrl,
        signatureAlgorithm: updated.signatureAlgorithm as any,
        identifierFormat: updated.identifierFormat || undefined,
        acceptedClockSkewMs: updated.acceptedClockSkewMs || undefined,
        attributeMapping: updated.attributeMapping as any,
