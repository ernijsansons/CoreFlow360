...
async function getHandler(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to process OT request' },
      { status: 500 }
    );
  }
}
...
async function handleGetDocument(docId: string, userId: string) {
  if (!docId || !userId) {
    return NextResponse.json(
      { error: 'docId and userId are required' },
      { status: 400 }
    );
  }
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        summary: {
          ...
          autoResolvedPercentage: statistics.totalConflicts > 0 
            ? Math.round((statistics.autoResolvedCount / statistics.totalConflicts) * 100)
            : 0,
          manualResolvedPercentage: statistics.totalConflicts > 0
  ...
  return NextResponse.json({
    ...
    data: {
      ...
      combined: {
        ...
        averageUsersPerDocument: otStats.activeDocuments > 0 
          ? Math.round(otStats.totalUsers / otStats.activeDocuments * 100) / 100
          : 0,
        conflictRate: otStats.totalOperations > 0
...
async function handleApplyOperation(data: any) {
  try {
    const { documentId, operation } = data;

    if (!documentId || !operation) {
      return NextResponse.json(
        { error: 'documentId and operation are required' },
        { status: 400 }
      );
    }
...
async function handleCreateDocument(data: any) {
  try {
    ...
    if (!docId || !type) {
      return NextResponse.json(
        { error: 'docId and type are required' },
        { status: 400 }
      );
    }
...
async function handleJoinDocument(data: any) {
  try {
...
async function handleLeaveDocument(data: any) {
  try {
    const { docId, userId } = data;
...
async function handleResolveConflict(data: any) {
  try {
...
async function handleDetectConflicts(data: any) {
  try {
    const {
      entityType,
      entityId,
    ...
    if (!entityType || !entityId || !localChanges || !remoteChanges || !baseState || !context) {
      return NextResponse.json(
        { error: 'Missing required fields: entityType, entityId, localChanges, remoteChanges, baseState, context' },
        { status: 400 }
      );
    }
