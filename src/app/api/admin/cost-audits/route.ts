...
export async function GET(request: NextRequest) {
  try {
    ...
    const auditResults = recentAudits.map(audit => {
      try {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        totalPotentialSavings: auditResults.reduce((sum, result) => sum + result.potentialSavings, 0),
        totalCriticalIssues: auditResults.reduce((sum, result) => sum + result.criticalIssues.length, 0),
        auditTypes: auditResults.map(result => result.auditType)
      }
    });
    ...
    return NextResponse.json(
      { error: 'Failed to execute cost audit' },
      { status: 500 }
    );
  }
}
