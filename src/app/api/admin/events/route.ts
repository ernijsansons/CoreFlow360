      ...
      data: {
        projections: {
          customerReadModel: {
            ...
            error: customerReadModel.status === 'rejected' ? customerReadModel.reason?.message : null
          },
          voiceAnalytics: {
          ...
          securityDashboard: {
            ...
            error: securityDashboard.status === 'rejected' ? securityDashboard.reason?.message : null
          }
        },
        summary: {
          totalProjections: 3,
          healthy: [customerReadModel, voiceAnalytics, securityDashboard]
...
async function handleAppendEvent(data: any) {
  try {
    const {
      aggregateId,
      aggregateType,
...
async function handleCreateSnapshot(data: any) {
  try {
...
async function handleRebuildProjections(data: any) {
  try {
    const { fromDate } = data;
