...
export async function GET(request: NextRequest) {
  try {
    ...
    const filter: APIKeyFilter = {
      ...
      vendor: searchParams.get('vendor')?.split(',').map(v => sanitizeInput(v)),
      securityLevel: searchParams.get('securityLevel')?.split(',') as any,
      searchTerm: searchParams.get('search') ? sanitizeInput(searchParams.get('search')!) : undefined,
      lastUsedSince: searchParams.get('lastUsedSince') ? new Date(searchParams.get('lastUsedSince')!) : undefined,
      expiringBefore: searchParams.get('expiringBefore') ? new Date(searchParams.get('expiringBefore')!) : undefined,
    ...
    await auditSecurityEvent('API_KEYS_LISTED', { 
    ...
    await auditSecurityEvent('API_KEYS_LIST_ERROR', { 
      error: error.message 
    }, request)
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!result.success) {
      await auditSecurityEvent('API_KEY_CREATE_FAILED', { 
    ...
    await auditSecurityEvent('API_KEY_CREATED', { 
      service: createRequest.service,
      name: createRequest.name,
    ...
    await auditSecurityEvent('API_KEY_CREATE_ERROR', { 
      error: error.message 
    }, request)
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.error('Bulk API key deletion error:', error)
