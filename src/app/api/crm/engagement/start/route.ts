...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to start engagement sequence' }, { status: 500 })
  }
}
...
async function generateTouchpointContent(_channel: string, _contactId: string) {
  ...
  return JSON.stringify(
    contentTemplates[channel as keyof typeof contentTemplates] || contentTemplates.email
  )
}
