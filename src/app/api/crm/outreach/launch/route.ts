...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to launch sequence' }, { status: 500 })
  }
}

async function scheduleOutreachStep(campaignId: string, step: unknown, targetProfile: unknown) {
...
async function executeOutreachMethod(
  _campaignId: string,
  _method: string,
