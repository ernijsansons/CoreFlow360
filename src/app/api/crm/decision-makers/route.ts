...
export async function GET(request: NextRequest) {
  try {
    ...
    const transformedExecutives = executives.map(exec => ({
      ...
      buyingSignals: includeSignals ? exec.buyingSignals?.map(signal => ({
        id: signal.id,
      ...
      responseRate: exec.responseHistory?.length > 0 ? 
        exec.responseHistory.filter(r => r.engaged).length / exec.responseHistory.length : 0,
      avgResponseTime: exec.responseHistory?.length > 0 ?
        exec.responseHistory.reduce((sum, r) => sum + r.responseTime, 0) / exec.responseHistory.length : 0,
    ...
    const highValueSignals = transformedExecutives.reduce((sum, exec) => 
      sum + (exec.buyingSignals?.filter(s => s.strength === 'HIGH' || s.strength === 'CRITICAL').length || 0), 0
    )
    const recentJobChanges = transformedExecutives.reduce((sum, exec) => 
      sum + (exec.recentJobChanges?.length || 0), 0
    )
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        averageInfluenceScore: Math.round(
          transformedExecutives.reduce((sum, exec) => sum + exec.influenceScore, 0) / totalExecutives
        ),
        signalDistribution: {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!tenantId || !linkedinUrl) {
      return NextResponse.json(
        { error: 'Tenant ID and LinkedIn URL required' },
        { status: 400 }
      )
    }
    ...
    if (existingExecutive) {
      return NextResponse.json(
        { error: 'Executive already being tracked' },
        { status: 409 }
      )
    }
    ...
    if (autoEnrich) {
      try {
        ...
        console.log('Auto-enriching executive profile:', linkedinId)
      } catch (enrichError) {
        console.warn('Failed to auto-enrich profile:', enrichError)
      }
    }
    ...
    setImmediate(async () => {
      try {
        ...
        console.error('Background monitoring error:', monitorError)
      }
    })
    ...
    console.error('Error adding executive to tracking:', error)
    return NextResponse.json(
      { error: 'Failed to add executive to tracking' },
      { status: 500 }
    )
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!executiveId || !tenantId) {
      return NextResponse.json(
        { error: 'Executive ID and tenant ID required' },
        { status: 400 }
      )
    }
    ...
    console.error('Error updating executive tracking:', error)
    return NextResponse.json(
      { error: 'Failed to update executive tracking' },
      { status: 500 }
    )
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.error('Error removing executive from tracking:', error)
    return NextResponse.json(
      { error: 'Failed to remove executive from tracking' },
      { status: 500 }
    )
  }
}