 ...
 */
export async function GET(request: NextRequest): Promise<NextResponse<ModuleResponse | { error: string }>> {
  ...
  try {
    ...
    const maxModules = user.subscription 
      ? tierLimits[user.subscription.tier] || 0 
      : 0;

    const activeModuleIds = user.subscription?.modules
      .filter(m => m.isActive)
      .map(m => m.moduleId) || [];

    const availableSlots = maxModules === -1 
      ? 999 
      : Math.max(0, maxModules - activeModuleIds.length);
    ...
    const response: ModuleResponse = {
      ...
      intelligenceImpact: {
        ...
        description: `Activating all available modules would ${
          potentialMultiplier > currentMultiplier * 2 
            ? 'exponentially' 
            : 'significantly'
        } increase your business intelligence by ${
          Math.round((potentialMultiplier / currentMultiplier - 1) * 100)
        }%`
      }
    };
 ...
 */
export async function POST(request: NextRequest): Promise<NextResponse<ModuleActivationResponse | { error: string }>> {
  const context: ErrorContext = {
    endpoint: '/api/consciousness/modules',
    method: 'POST',
    userAgent: request.headers.get('user-agent') || undefined,
