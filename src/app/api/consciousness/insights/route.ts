...
export async function GET(
  ...
  try {
    ...
    const summary = {
      ...
      last24h: insights.filter((i) => new Date(i.generatedAt).getTime() > now - day).length,
      last7days: insights.filter((i) => new Date(i.generatedAt).getTime() > now - week).length,
      byCategory: insights.reduce(
        (acc, insight) => {
...
async function generateInsights(
  ...
  filteredInsights = filteredInsights.filter(
    (i) => new Date(i.generatedAt).getTime() > Date.now() - timeLimit
  )
...
function generateModuleInsights(
  ...
  if (moduleId === 'crm') {
    insights.push({
      ...
      description:
        'Customers who engage with support within first 7 days have 85% higher retention rate',
      confidence: 0.92,
      impact: 'high',
      modules: ['crm'],
      visualizations: [
...
function generateCrossModuleInsights(
  ...
  if (modules.includes('crm') && modules.includes('accounting')) {
    insights.push({
      ...
      description:
        'Customer satisfaction scores directly correlate with payment speed - 1 point increase = 3.2 days faster payment',
      confidence: 0.89,
      impact: 'high',
...
function analyzeTrends(_insights: ConsciousnessInsight[], _tier: string): unknown[] {
  ...
  for (const category of categories) {
    ...
    const recentCount = categoryInsights.filter(
      (i) => new Date(i.generatedAt).getTime() > Date.now() - 24 * 60 * 60 * 1000
    ).length
