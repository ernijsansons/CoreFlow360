...
export async function POST(request: NextRequest) {
  try {
    ...
    const pricingResponse = await fetch(
      `${process.env.NEXT_PUBLIC_BASE_URL}/api/pricing/calculate`,
      {
        method: 'POST',
    ...
    return NextResponse.json({
      ...
      estimatedTotal:
        billingCycle === 'annual' ? pricing.totalAnnualPrice : pricing.totalMonthlyPrice,
      currency: 'USD',
      metadata: {
        tenantId,
        modules,
...
async function createStripeLineItems({
  ...
  if (bundleKey) {
    ...
    const bundle = bundleData.bundles?.find((b: unknown) => b.bundleKey === bundleKey)
