...
function generateOnboardingInsights(onboarding: unknown, stepProgress: unknown) {
  const insights = []
  const completedSteps = JSON.parse(onboarding.completedSteps || '[]')
...
function generateRecommendations(_onboarding: unknown, _stepProgress: unknown) {
...
function getNextStep(onboarding: unknown, completedSteps: string[]) {
...
function getEstimatedTimeRemaining(onboarding: unknown): string {
...
function getPostOnboardingSteps(_role: string): string[] {
  return [
...
function calculateTotalTimeSpent(stepProgress: unknown): number {
  return Object.values(stepProgress).reduce((total: number, step: unknown) => {
...
function getRoleSpecificInsights(_role: string, _completedSteps: string[]) {
