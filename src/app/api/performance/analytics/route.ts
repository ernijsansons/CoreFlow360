  ...
  try {
    ...
    if (!query.metric || query.metric === 'database') {
      response.database = {
        ...
        performance: {
          status:
            analytics.metrics.database.avgQueryTime < 100
              ? 'EXCELLENT'
              : analytics.metrics.database.avgQueryTime < 500
                ? 'GOOD'
                : analytics.metrics.database.avgQueryTime < 1000
                  ? 'FAIR'
                  : 'POOR',
    ...
    if (!query.metric || query.metric === 'cache') {
      response.cache = {
        ...
        performance: {
          status:
            analytics.metrics.cache.hitRatio > 80
              ? 'EXCELLENT'
              : analytics.metrics.cache.hitRatio > 60
                ? 'GOOD'
                : analytics.metrics.cache.hitRatio > 40
                  ? 'FAIR'
                  : 'POOR',
    ...
    response.insights = {
      ...
      keyMetrics: {
        responseTime: {
          ...
          target: 200,
          status:
            analytics.metrics.overview.averageResponseTime < 200
              ? 'ON_TARGET'
              : 'NEEDS_IMPROVEMENT',
        },
        throughput: {
...
function calculateHealthScore(metrics: unknown): number {
  const responseTimeScore =
    metrics.overview.averageResponseTime < 200
      ? 100
      : metrics.overview.averageResponseTime < 500
        ? 80
        : metrics.overview.averageResponseTime < 1000
          ? 60
          : 40

  const errorRateScore =
    metrics.overview.errorRate < 1
      ? 100
      : metrics.overview.errorRate < 2
        ? 80
        : metrics.overview.errorRate < 5
          ? 60
          : 40

  const cacheScore =
    metrics.cache.hitRatio > 80
      ? 100
      : metrics.cache.hitRatio > 60
        ? 80
        : metrics.cache.hitRatio > 40
          ? 60
          : 40

  const poolScore =
    metrics.database.poolUtilization < 70
      ? 100
      : metrics.database.poolUtilization < 85
        ? 80
        : metrics.database.poolUtilization < 95
          ? 60
          : 40
...
function getSystemStatus(metrics: unknown): string {
...
function identifyDatabaseBottlenecks(dbMetrics: unknown): string[] {
  const bottlenecks = []
...
function getDatabaseOptimizations(dbMetrics: unknown): string[] {
  const optimizations = []
...
function calculateCacheEfficiency(cacheMetrics: unknown): number {
  const hitRatioScore = cacheMetrics.hitRatio
  const evictionScore =
    cacheMetrics.evictions < 10
      ? 100
      : cacheMetrics.evictions < 50
        ? 80
...
function getCacheRecommendations(cacheMetrics: unknown): string[] {
...
function getSystemHealthStatus(systemMetrics: unknown): string {
  const avgUsage =
    (systemMetrics.cpuUsage + systemMetrics.memoryUsage + systemMetrics.diskUsage) / 3
...
function generatePerformanceSummary(metrics: unknown): string {
  const healthScore = calculateHealthScore(metrics)
...
): Array<{ title: string; impact: string; effort: string }> {
  return recommendations
