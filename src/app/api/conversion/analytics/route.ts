...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(
      `📊 Getting conversion analytics for ${tenantId || 'global'}, timeframe: ${timeframe}`
    )
    ...
    return NextResponse.json({ error: 'Failed to get conversion analytics' }, { status: 500 })
  }
}
...
function calculateFunnelMetrics(events: unknown[]) {
  const funnelSteps = [
    'role_selected',
    'agent_selected',
    'feature_usage',
  ...
  return {
    ...
    overallConversionRate:
      funnel.length > 0
        ? Math.round((funnel[funnel.length - 1].users / funnel[0].users) * 10000) / 100
        : 0,
  }
}

function calculateConversionRates(events: unknown[]) {
  ...
  const byUserPlan = events.reduce(
    (acc, event) => {
  ...
  return {
    ...
    byEventType: Object.entries(byEventType).map(([type, data]: [string, unknown]) => ({
      eventType: type,
      conversionRate: data.total > 0 ? Math.round((data.converted / data.total) * 10000) / 100 : 0,
      totalEvents: data.total,
      conversions: data.converted,
    })),
    byUserPlan: Object.entries(byUserPlan).map(([plan, data]: [string, unknown]) => ({
      userPlan: plan,
      conversionRate: data.total > 0 ? Math.round((data.converted / data.total) * 10000) / 100 : 0,
      totalEvents: data.total,
      conversions: data.converted,
...
function calculateSegmentAnalysis(events: unknown[], segment: string) {
...
function calculateTimeSeriesData(events: unknown[], timeframe: string) {
...
function calculateTopPerformers(events: unknown[]) {
  ...
  const topModules = Object.entries(modules)
    .map(([module, data]: [string, unknown]) => ({
      module,
      conversionRate: data.total > 0 ? Math.round((data.converted / data.total) * 10000) / 100 : 0,
      totalEvents: data.total,
      conversions: data.converted,
...
function calculateUserJourneys(events: unknown[]) {
  ...
  const conversionTimes = Object.values(userJourneys)
    .filter((journey: unknown[]) => journey.some((e) => e.actionTaken === 'converted'))
    .map((journey: unknown[]) => {
      const firstEvent = journey.sort(
      ...
      if (firstEvent && conversionEvent) {
        return (
          new Date(conversionEvent.createdAt).getTime() - new Date(firstEvent.createdAt).getTime()
        )
      }
      return null
    })
  ...
  const paths = Object.values(userJourneys).map((journey: unknown[]) =>
    journey
  ...
  // Drop-off points
  const dropoffPoints = events.reduce(
    (acc, event) => {
...
function generateConversionInsights(
  funnelMetrics: unknown,
...
function generateConversionRecommendations(_funnelMetrics: unknown, _conversionRates: unknown) {
...
function assessDataQuality(events: unknown[]): string {
...
async function exportConversionData(filters: unknown, format: string) {
...
async function generateCohortAnalysis(_filters: unknown) {
...
async function generateAttributionAnalysis(_filters: unknown) {
