...
async function handler(request: NextRequest) {
  try {
    ...
    if (!tenantId || !idpName) {
      return NextResponse.json(
        { error: 'Missing tenant or IdP information' },
        { status: 400 }
      );
    }
    ...
    const isValid = await samlAuth.validateResponseSignature(
      samlResponse,
      tenantId,
      idpName
    );
    ...
    const user = await new Promise((resolve, reject) => {
      ...
      strategy.authenticate(mockReq as any, {
        success: (user: any) => resolve(user),
        error: (err: any) => reject(err),
        fail: (info: any) => reject(new Error(info || 'Authentication failed'))
      });
    });
    ...
    return NextResponse.redirect(
      new URL(`/auth/error?error=saml_callback_failed`, request.url)
    );
  }
}
