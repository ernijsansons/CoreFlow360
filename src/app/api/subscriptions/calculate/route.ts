...
const PROMO_CODES = {
  ...
  'ENTERPRISE20': { discount: 0.20, validUntil: new Date('2024-06-30'), minSpend: new Decimal('500') },
  'STARTUP15': { discount: 0.15, validUntil: new Date('2025-01-31'), minSpend: new Decimal('50') },
  'PARTNER10': { discount: 0.10, validUntil: new Date('2025-12-31'), minSpend: new Decimal('25') }
} as const
  ...
  try {
    ...
    if (bundles.length === 0) {
      return NextResponse.json(
        { error: 'No valid bundles found', warnings },
        { status: 400 }
      )
    }
    ...
    const bundleSubtotal = breakdown.reduce(
    ...
    if (bundles.length >= 3) {
      discountPercentage = Math.max(discountPercentage, 0.20)
      savings.multiBundle = subtotal.mul(0.20)
    ...
    if (users >= 100) {
      ...
      discountPercentage = Math.max(discountPercentage, 0.10)
      savings.volume = subtotal.mul(0.10)
    ...
    if (promoCode && PROMO_CODES[promoCode]) {
      ...
      if (now <= promo.validUntil && subtotal.gte(promo.minSpend)) {
        ...
        recommendations.push(`Promotional code ${promoCode} applied: ${Math.round(promo.discount * 100)}% off`)
    ...
    if (!annual) {
      ...
      recommendations.push(`Switch to annual billing to save approximately $${annualSavings.toFixed(2)} per year`)
    }
