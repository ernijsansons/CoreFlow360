...
const BOMOptimizationSchema = z.object({
  ...
  optimization_criteria: z
    .enum(['cost', 'quality', 'sustainability', 'lead_time', 'balanced'])
    .default('balanced'),
  tenant_id: z.string().min(1),
  user_id: z.string().min(1),
})
...
class ERPNextServiceIntegration {
  ...
  async executePythonService(request: unknown): Promise<unknown> {
...
asyncio.run(execute())
      ...
      pythonProcess.on('close', (code) => {
        ...
        if (code === 0) {
          ...
          reject(
            new Error(`ERPNext process failed with code ${code}: ${stderr || 'Unknown error'}`)
          )
        }
      })
