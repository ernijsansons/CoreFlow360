...
const TimeEntrySchema = z.object({
  ...
  activity_type: z
    .enum(['research', 'client_work', 'court_work', 'administrative'])
    .default('client_work'),
...
class DolibarrServiceIntegration {
  ...
  async executeNodeService(request: unknown): Promise<unknown> {
    return new Promise((resolve, reject) => {
      ...
      nodeProcess.on('close', (code) => {
        ...
        if (code === 0) {
          ...
          reject(
            new Error(`Dolibarr process failed with code ${code}: ${stderr || 'Unknown error'}`)
          )
        }
      })
  ...
  async executePHPService(request: unknown): Promise<unknown> {
    return new Promise((resolve, reject) => {
      const startTime = Date.now()
      ...
      phpProcess.on('close', (code) => {
        ...
        if (code === 0) {
          ...
          reject(
            new Error(`Dolibarr PHP process failed with code ${code}: ${stderr || 'Unknown error'}`)
          )
        }
      })
