...
import {
  serviceHealthCheck,
  getServiceStats,
} from '@/lib/external-services/resilient-service-wrapper'
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const filteredStats =
      serviceName && allStats[serviceName] ? { [serviceName]: allStats[serviceName] } : allStats
    ...
    const response: unknown = {
      ...
      summary: {
        ...
        halfOpenBreakers: Object.values(filteredStats).filter((s) => s.state === 'HALF_OPEN')
          .length,
        averageErrorRate:
          Object.values(filteredStats).reduce((sum, s) => sum + s.errorRate, 0) /
          Object.keys(filteredStats).length,
      },
    }
 ...
 */
function generateRecommendations(stats: Record<string, unknown>): string[] {
