...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = {
      ...
      integration: {
        webhookEndpoint: validatedData.webhookUrl
          ? undefined
          : `/api/intelligence/ingestion/webhook`,
        webhookInstructions: validatedData.webhookUrl
          ? undefined
          : {
              method: 'POST',
              headers: {
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to retrieve data sources' }, { status: 500 })
  }
}
