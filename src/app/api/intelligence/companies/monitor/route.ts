...
export async function POST(request: NextRequest) {
  try {
    ...
    await discoveryEngine.startCompanyMonitoring({
      ...
      companyDomain: validatedData.companyDomain,
      industryType: validatedData.industryType,
      dataSources: validatedData.enabledSources as unknown[],
      frequency: validatedData.analysisFrequency as unknown,
    })
    ...
    await orchestrator.startCompanyMonitoring({
      ...
      companyDomain: validatedData.companyDomain,
      industryType: validatedData.industryType,
      enabledAgents: validatedData.enabledSources as unknown[],
      monitoringDepth: validatedData.monitoringDepth,
      alertThresholds: validatedData.alertThresholds,
    })
    ...
    await prisma.intelligenceReport.create({
      data: {
        ...
        dataSourcesUsed: validatedData.enabledSources as unknown[],
        dataQualityScore: 85,
  ...
  try {
    ...
    if (validatedData.monitoringStatus === 'PAUSED') {
      ...
      const agents = orchestrator
        .getAgentsStatus()
        .filter((agent) => agent.id.endsWith(`_${validatedData.companyIntelligenceId}`))
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Build query
    const where: unknown = { tenantId: session.user.tenantId }
    ...
    const companiesWithStatus = companies.map((company) => {
      ...
      const companyAgents = allAgents.filter(
        (agent) => agent.id.includes(company.id) || agent.company === company.companyName
      )

      const activeProblems = includeProblems ? company.problems || [] : []
      const criticalProblems = activeProblems.filter(
    ...
    const summary = {
      ...
      avgHealthScore:
        companies.reduce((sum, c) => sum + (c.overallHealthScore || 0), 0) / companies.length,
    }
    ...
    return NextResponse.json({ error: 'Failed to retrieve monitored companies' }, { status: 500 })
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const agents = orchestrator
      .getAgentsStatus()
      .filter((agent) => agent.id.endsWith(`_${companyId}`))
    agents.forEach((agent) => {
      orchestrator.toggleAgent(agent.id, false)
    })
    ...
    return NextResponse.json({ error: 'Failed to stop company monitoring' }, { status: 500 })
  }
}
