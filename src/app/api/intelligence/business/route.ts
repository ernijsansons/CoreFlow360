  ...
  try {
    ...
    switch (action) {
              ...
              trendDirection:
                forecast.predictions.length > 1
                  ? forecast.predictions[forecast.predictions.length - 1].predictedValue >
                    forecast.predictions[0].predictedValue
                    ? 'increasing'
                    : 'decreasing'
                  : 'stable',
              averageConfidence:
      ...
      case 'simulate':
        ...
        const simulationData = await runBusinessSimulation(
          engine,
          tenantId,
...
async function runBusinessSimulation(
  ...
  for (let i = 0; i < 30; i++) {
    ...
    const value = Math.min(
      1,
      baseEfficiency * (1 + params.efficiencyGain * (i / 30)) * (1 + dailyVariation)
    )
  ...
  console.log(
    `✅ Business simulation completed: ${metrics.length} metrics, ${forecasts.length} forecasts, ${insights.length} insights`
  )
