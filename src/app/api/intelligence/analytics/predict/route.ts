...
const predictProblemSchema = z.object({
  ...
  predictionTypes: z.array(z.enum(['ESCALATION', 'BUSINESS_IMPACT', 'SOLUTION_OUTCOME', 'TIMELINE'])).optional()
})
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('🔮 Generating problem predictions for:', validatedData.problemId)
    ...
    const response = {
      ...
      predictions: {
        escalationRisk: {
          ...
          riskLevel: predictions.predictions.escalationRisk.probability > 70 ? 'HIGH' : 
                    predictions.predictions.escalationRisk.probability > 40 ? 'MEDIUM' : 'LOW',
          ...
          mitigationActions: predictions.predictions.escalationRisk.probability > 70 ? [
        ...
        businessImpact: {
          financial: {
            ...
            opportunityCost: Math.round(predictions.predictions.businessImpact.revenueAtRisk * 0.15),
          ...
          operational: {
            ...
            productivityLoss: Math.round(predictions.predictions.businessImpact.operationalImpact * 1000) // hours
          },
          strategic: {
            reputationRisk: predictions.predictions.businessImpact.customerChurnRisk > 60 ? 'HIGH' : 'MEDIUM',
            competitiveDisadvantage: predictions.predictions.businessImpact.complianceRisk > 50,
            marketPositionImpact: predictions.predictions.businessImpact.revenueAtRisk > 100000 ? 'SIGNIFICANT' : 'MODERATE'
          }
        },
        ...
        solutionOutcome: {
          successMetrics: {
            ...
            confidenceLevel: predictions.predictions.solutionOutcome.successProbability > 80 ? 'HIGH' : 
                           predictions.predictions.solutionOutcome.successProbability > 60 ? 'MEDIUM' : 'LOW',
          ...
          risks: {
            ...
            technicalRisks: predictions.predictions.solutionOutcome.riskFactors.filter(r => 
              r.toLowerCase().includes('technical') || r.toLowerCase().includes('complexity')
            ),
            businessRisks: predictions.predictions.solutionOutcome.riskFactors.filter(r => 
              r.toLowerCase().includes('business') || r.toLowerCase().includes('competitor')
            )
          },
          alternatives: {
            ...
            recommendedApproach: predictions.predictions.solutionOutcome.successProbability > 80 ? 
              'AGGRESSIVE' : predictions.predictions.solutionOutcome.successProbability > 60 ? 'BALANCED' : 'CONSERVATIVE'
          }
        },
        ...
        timelineForecasting: {
          decisionPhase: {
            ...
            urgencyLevel: predictions.predictions.timelineForecasting.daysToDecision < 7 ? 'CRITICAL' :
                         predictions.predictions.timelineForecasting.daysToDecision < 21 ? 'HIGH' : 'MEDIUM',
            keyMilestones: [
          ...
          implementationPhase: {
            ...
            complexity: predictions.predictions.timelineForecasting.implementationDuration > 180 ? 'HIGH' :
                       predictions.predictions.timelineForecasting.implementationDuration > 60 ? 'MEDIUM' : 'LOW',
            resourceRequirements: predictions.predictions.timelineForecasting.implementationDuration > 120 ? [
          ...
          returnOnInvestment: {
            ...
            breakEvenPoint: new Date(Date.now() + predictions.predictions.timelineForecasting.paybackPeriod * 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
            longTermValue: predictions.predictions.solutionOutcome.expectedROI > 200 ? 'HIGH' : 'MEDIUM'
          }
        }
      },
      ...
      intelligence: {
        ...
        actionUrgency: predictions.predictions.timelineForecasting.daysToDecision < 14 ? 'IMMEDIATE' : 'PLANNED',
        strategicValue: predictions.predictions.businessImpact.revenueAtRisk > 100000 ? 'HIGH' : 'MEDIUM',
      ...
      recommendations: validatedData.includeRecommendations ? {
        immediate: predictions.recommendations.filter(r => r.type === 'IMMEDIATE').map(r => ({
          action: r.action,
          priority: r.priority,
        ...
        shortTerm: predictions.recommendations.filter(r => r.type === 'SHORT_TERM').map(r => ({
          action: r.action,
          priority: r.priority,
          reasoning: r.reasoning,
          expectedOutcome: r.expectedOutcome,
        ...
        longTerm: predictions.recommendations.filter(r => r.type === 'LONG_TERM').map(r => ({
          action: r.action,
          priority: r.priority,
          reasoning: r.reasoning,
          expectedOutcome: r.expectedOutcome,
    ...
    console.error('❌ Failed to generate problem predictions:', error)
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (analysisType === 'trends') {
      ...
      const industryType = searchParams.get('industry') as any
        ...
        data: {
          ...
          forecasts: {
            next30Days: {
              ...
              riskLevel: trendAnalysis.forecast.next30Days.expectedProblems > 30 ? 'HIGH' : 'MEDIUM'
            },
            next90Days: {
            ...
            longTerm: {
              ...
              investmentRecommendations: trendAnalysis.forecast.next365Days.investmentRecommendations
            }
          }
        }
      })
      ...
      return NextResponse.json({
        ...
        availableAnalytics: {
          problemPredictions: {
            ...
            capabilities: ['Escalation risk', 'Business impact', 'Solution outcome', 'Timeline forecasting']
          },
          trendAnalysis: {
        ...
        supportedPredictionTypes: [
          'ESCALATION', 'BUSINESS_IMPACT', 'SOLUTION_OUTCOME', 'TIMELINE'
        ],
        supportedTrendPeriods: [
