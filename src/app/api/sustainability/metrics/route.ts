...
async function calculateSustainabilityMetrics(
  ...
  const avgBundleSize = performanceMetrics.length > 0
    ? performanceMetrics.reduce((sum, m) => sum + (m.bundleSize || 0), 0) / performanceMetrics.length
    : 2500 // Default estimate in KB

  const avgLoadTime = performanceMetrics.length > 0
    ? performanceMetrics.reduce((sum, m) => sum + (m.loadTime || 0), 0) / performanceMetrics.length
    : 1200 // Default estimate in ms

  const avgRenderTime = performanceMetrics.length > 0
    ? performanceMetrics.reduce((sum, m) => sum + (m.renderTime || 0), 0) / performanceMetrics.length
  ...
  const darkModeUsers = userPreferences.filter(u => 
    u.themePreferences && 
    (u.themePreferences as any).mode === 'dark' || 
    (u.themePreferences as any).mode === 'system'
  ).length

  const reducedMotionUsers = userPreferences.filter(u =>
    u.themePreferences &&
    (u.themePreferences as any).accessibilityMode === 'reduced-motion' ||
    !(u.themePreferences as any).animations
  ).length
