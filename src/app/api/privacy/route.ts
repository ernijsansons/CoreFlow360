...
async function getComplianceStatus(_tenantId: string, _jurisdictions: string[]) {
...
async function submitDataSubjectRequest(request: NextRequest, tenantId: string, userId: string) {
  ...
  return NextResponse.json({
    ...
    requestId,
    message:
      'Data subject request submitted successfully. You will receive a verification email shortly.',
  })
}
...
async function verifyDataSubjectRequest(request: NextRequest, tenantId: string) {
  ...
  if (!requestId || !verificationCode) {
    return NextResponse.json(
      { error: 'requestId and verificationCode are required' },
      { status: 400 }
    )
  }
