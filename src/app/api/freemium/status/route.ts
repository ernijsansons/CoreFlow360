...
async function getFreemiumStatus(request: NextRequest) {
  try {
    ...
    const freemiumUser = await queryCache.subscription(userId, () =>
    ...
    const daysSinceJoined = Math.ceil(
      (new Date().getTime() - new Date(updatedFreemiumUser.createdAt).getTime()) /
        (1000 * 60 * 60 * 24)
    )
    const averageUsagePerDay =
      updatedFreemiumUser.daysActive > 0
        ? updatedFreemiumUser.weeklyUsageCount / Math.min(updatedFreemiumUser.daysActive, 7)
        : 0
    ...
    return NextResponse.json({ error: 'Failed to get freemium status' }, { status: 500 })
  }
}
...
function getUpgradeRecommendations(_freemiumUser: unknown, _recentEvents: unknown[]): string[] {
...
function shouldShowUpgradePrompt(_freemiumUser: unknown, _recentEvents: unknown[]): boolean {
  // Don't prompt too frequently
  if (freemiumUser.lastUpgradePrompt) {
    const daysSinceLastPrompt =
      (new Date().getTime() - new Date(freemiumUser.lastUpgradePrompt).getTime()) /
      (1000 * 60 * 60 * 24)
