...
test.describe('Voice Dictation E2E Flow', () => {
      ...
      window.WebSocket = class MockWebSocket {
        ...
        simulateTranscription() {
          const transcriptions = [
            ...
            { type: 'transcription', transcript: 'Hello, this is', isFinal: false, confidence: 0.85 },
            { type: 'transcription', transcript: 'Hello, this is a test', isFinal: true, confidence: 0.95 },
            { type: 'transcription', transcript: 'transcription', isFinal: false, confidence: 0.9 },
            { type: 'transcription', transcript: 'transcription for the voice', isFinal: false, confidence: 0.92 },
            { type: 'transcription', transcript: 'transcription for the voice note system.', isFinal: true, confidence: 0.98 }
          ]
  ...
  test('Complete voice dictation flow - record, transcribe, save', async () => {
    ...
    await expect(transcriptionBox).toContainText('Hello, this is a test transcription for the voice note system.')
  ...
  test('Error handling - microphone permission denied', async () => {
    ...
    await expect(page.locator('[data-testid="error-message"]')).toContainText(/microphone access denied/i)
  ...
  test('Mobile responsive behavior', async () => {
    ...
    const transcriptionBox = page.locator('[data-testid="transcription-box"]')
    const transcriptionStyle = await transcriptionBox.evaluate(el => 
      window.getComputedStyle(el)
    )
    
    // Should have mobile-optimized scrolling
    expect(transcriptionStyle.webkitOverflowScrolling || transcriptionStyle.overflowScrolling).toBe('touch')
  ...
  test('Accessibility compliance', async () => {
    ...
    const buttonStyle = await recordButton.evaluate(el => 
      window.getComputedStyle(el)
    )
