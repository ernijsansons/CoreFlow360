...
describe('Database Tenant Isolation', () => {
  ...
  describe('Customer Data Isolation', () => {
    it('should only return customers for the requesting tenant', async () => {
      ...
      const tenant2Customers = [{ id: 'cust-3', name: 'Customer 3', tenantId: tenant2Id }]
  ...
  describe('AI Insights and Decisions Isolation', () => {
    ...
    it('should prevent accessing AI decisions across tenants', async () => {
      ...
      await expect(getDecisionForTenant('decision-1', tenant2Id)).rejects.toThrow(
  ...
  describe('Security Boundary Enforcement', () => {
    it('should validate tenant context in all operations', async () => {
      ...
      const secureFind = ensureTenantContext(mockPrisma.customer.findMany, tenant1Id)

      // Test missing tenant context
      await expect(secureFind({ where: {} })).rejects.toThrow(
        'Tenant context required for all operations'
      )

      // Test wrong tenant context
      await expect(secureFind({ where: { tenantId: tenant2Id } })).rejects.toThrow(
        'Operation not allowed for different tenant'
      )
    })
    ...
    it('should log tenant isolation violations', async () => {
      ...
      await expect(validateTenantAccess('cust-1', tenant2Id)).rejects.toThrow(
