...
import { NocoBaseOrchestrator } from '@/lib/integrations/nocobase/plugin-orchestrator'
import { BigcapitalAccountingPlugin } from '@/lib/integrations/bigcapital/bigcapital-plugin'
import { TwentyCRMPlugin } from '@/lib/integrations/twenty/twenty-crm-plugin'
import { CoreFlowEventBus, EventType, EventChannel } from '@/lib/core/events/event-bus'
import { AIAgentOrchestrator } from '@/lib/ai/orchestration/ai-agent-orchestrator'
...
describe('NocoBase Orchestrator Integration', () => {
  ...
  describe('Plugin Registration', () => {
    ...
    it('should reject invalid plugins', async () => {
      ...
      await expect(orchestrator.registerPlugin(invalidPlugin)).rejects.toThrow(
  ...
  describe('AI Task Integration', () => {
    it('should submit AI tasks through orchestrator API', async () => {
      ...
      const submitSpy = jest.spyOn(aiOrchestrator, 'submitTask').mockResolvedValue('task_123')
