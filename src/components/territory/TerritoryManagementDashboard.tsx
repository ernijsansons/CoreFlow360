...
export default function TerritoryManagementDashboard({
  ...
  const [viewMode, setViewMode] = useState<
    'overview' | 'territory' | 'leads' | 'analytics' | 'mobile' | 'manager' | 'coaching'
  >('overview')
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-gray-600">AI-powered territory optimization and lead routing</p>
        </div>

        <div className="flex items-center space-x-3">
          {/* View Mode Toggle */}
          <div className="inline-flex rounded-md shadow-sm">
            {(['overview', 'analytics', 'mobile', 'manager', 'coaching'] as const).map(
              (mode, index, array) => (
                <button
                  key={mode}
          ...
          <button className="inline-flex items-center rounded-md bg-gradient-to-r from-blue-600 to-purple-600 px-4 py-2 text-white shadow-sm hover:from-blue-700 hover:to-purple-700">
        ...
        <>
                  ...
                  <div className="ml-5 w-0 flex-1">
                    <dl>
                      ...
                      <dd className="text-lg font-medium text-gray-900">
                        {Math.round(
                          (territories.reduce((acc, t) => acc + t.leadConversionRate, 0) /
                            territories.length) *
                            100
                        )}
                        %
            ...
            >
              <div className="p-5">
                <div className="flex items-center">
                  ...
                  <div className="ml-5 w-0 flex-1">
                    <dl>
                      <dt className="truncate text-sm font-medium text-gray-500">Pipeline Value</dt>
                      <dd className="text-lg font-medium text-gray-900">
                        {formatCurrency(
                          territoryAnalytics.reduce((acc, a) => acc + a.pipelineValue, 0)
                        )}
                      </dd>
                    </dl>
                  </div>
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            <div className="rounded-lg bg-white shadow">
              <div className="border-b border-gray-200 p-6">
                ...
                <p className="mt-1 text-sm text-gray-600">
                  Visual territory boundaries and lead distribution
                </p>
              </div>
              <div className="p-6">
            ...
            <div className="rounded-lg bg-white shadow">
              <div className="border-b border-gray-200 p-6">
                <h3 className="flex items-center text-lg font-medium text-gray-900">
                  <ChartBarIcon className="mr-2 h-5 w-5 text-green-600" />
                  Territory Performance
                </h3>
                <p className="mt-1 text-sm text-gray-600">AI-powered performance analytics</p>
              </div>
              <div className="p-6">
                  ...
                  {territories.map((territory) => (
                    ...
                    >
                      ...
                      <div className="flex items-center space-x-2">
                        <span
                          className={`rounded-full px-2 py-1 text-xs ${getPerformanceColor(territory.leadConversionRate / 0.4)}`}
                        >
                          {territory.leadConversionRate > 0.3
                            ? 'Excellent'
                            : territory.leadConversionRate > 0.2
                              ? 'Good'
                              : 'Needs Focus'}
                        </span>
                        <button
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            <div className="rounded-lg bg-white shadow">
              <div className="border-b border-gray-200 p-6">
                ...
                <p className="mt-1 text-sm text-gray-600">AI-routed leads by territory</p>
              </div>
              <div className="p-6">
                <div className="space-y-4">
            ...
            <div className="rounded-lg bg-white shadow">
              <div className="border-b border-gray-200 p-6">
                ...
                <p className="mt-1 text-sm text-gray-600">AI-optimized visit schedule</p>
              </div>
              <div className="p-6">
                <div className="space-y-4">
                  {territories
                    .filter((t) => t.nextVisitDate)
                    .map((territory) => (
                      <div
                        key={territory.id}
                        className="flex items-center justify-between rounded-lg border border-gray-200 p-3"
                      >
                        <div>
                          <h4 className="font-medium text-gray-900">{territory.name}</h4>
                          <p className="text-sm text-gray-600">
                            {formatDate(territory.nextVisitDate!)} • {territory.visitFrequency}{' '}
                            frequency
                          </p>
                          <div className="mt-1 flex items-center space-x-2">
          ...
          <div>
            ...
            <div className="mt-2 space-y-1">
              <p className="text-sm text-gray-700">
                • Consider increasing Northeast Hub visit frequency to 2x weekly (expected 25%
                revenue increase)
              </p>
              <p className="text-sm text-gray-700">
                • Route optimization could reduce travel time by 18% (saving $450/month in fuel
                costs)
              </p>
              <p className="text-sm text-gray-700">
                • 3 high-value leads await assignment - auto-routing recommended
              </p>
            </div>
          </div>
          <button className="rounded-lg bg-gradient-to-r from-blue-600 to-purple-600 px-6 py-3 text-white transition-all hover:from-blue-700 hover:to-purple-700">
            Apply Suggestions
          </button>
        </div>
      </div>
