        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center space-x-3">
              <SafeText className="truncate text-sm font-medium text-gray-900">
            ...
            <div className="mt-1 flex items-center space-x-4">
              {customer.company && (
                <SafeText className="text-sm text-gray-500">{customer.company}</SafeText>
              )}
              <p className="text-sm text-gray-500">
                {formatCurrency(customer.aiLifetimeValue)} LTV
              </p>
        ...
        <div className="flex items-center space-x-6">
          ...
          <div className="flex items-center space-x-4 text-sm">
            <div className="text-center">
              ...
              <p className="font-medium text-gray-900">{formatCurrency(customer.totalRevenue)}</p>
            </div>
            <div className="text-center">
...
export default function CRMDashboard() {
  ...
  const calculateMetricsFromCustomers = useCallback((customersList: Customer[]): CRMMetrics => {
    ...
    const statusCounts = customersList.reduce(
      (acc, customer) => {
    ...
    const potentialCustomers =
      (statusCounts.LEAD || 0) + (statusCounts.PROSPECT || 0) + (statusCounts.CUSTOMER || 0)
    const conversionRate =
      potentialCustomers > 0 ? ((statusCounts.CUSTOMER || 0) / potentialCustomers) * 100 : 0
    ...
    const churnRisk =
      activeCustomers > 0 ? (atRiskCustomers / (activeCustomers + atRiskCustomers)) * 100 : 0
  ...
  const getStatusTextColor = useCallback((_status: string) => {
    const stage = lifecycleStages.find((s) => s.id === status)
      ...
      <div className="flex h-64 flex-col items-center justify-center space-y-4">
        <div className="max-w-md rounded-lg bg-red-50 p-6">
          <div className="flex items-start space-x-3">
            <ExclamationCircleIcon className="mt-0.5 h-6 w-6 text-red-600" />
            <div>
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="inline-flex rounded-md shadow-sm" role="group">
            ...
            <button
              ...
              className={`border-t border-b border-gray-300 px-4 py-2 text-sm font-medium ${
                viewMode === 'map'
                  ? 'border-blue-600 bg-blue-600 text-white'
                  : 'bg-white text-gray-900 hover:bg-gray-50'
              }`}
            ...
            <button
              ...
              className={`rounded-r-md border border-gray-300 px-4 py-2 text-sm font-medium ${
                viewMode === 'territory'
                  ? 'border-blue-600 bg-blue-600 text-white'
                  : 'bg-white text-gray-900 hover:bg-gray-50'
              }`}
      ...
      {viewMode === 'map' && (
        <div className="space-y-6">
          ...
          <CustomerLocationManager showMap={false} onLocationSelect={(location) => {}} />
        </div>
      )}
      ...
      <CreateCustomerModal isOpen={showCreateModal} onClose={() => setShowCreateModal(false)} />
    </div>
  )
}
