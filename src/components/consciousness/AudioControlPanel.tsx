  ...
  return (
    <div className={`fixed ${positionClasses[position]} z-50 ${className}`}>
      <motion.div
        ...
        className="overflow-hidden rounded-2xl border border-gray-700 bg-black/80 shadow-2xl backdrop-blur-xl"
      >
        {/* Main Control Button */}
        <div className="relative h-full w-full">
          <button
            onClick={() => setIsExpanded(!isExpanded)}
            className="absolute top-3 left-3 flex h-12 w-12 items-center justify-center rounded-xl border border-cyan-400/30 bg-gradient-to-r from-cyan-500/20 to-purple-500/20 text-white transition-all hover:border-cyan-400/50"
          ...
          {showConsciousnessLevel && (
            <div className="absolute top-3 right-3 h-12 w-6">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className={`h-6 w-12 rounded-full transition-colors ${
                        consciousnessAudio.isAudioEnabled ? 'bg-cyan-500' : 'bg-gray-600'
                      }`}
                    >
                      <div
                        className={`h-5 w-5 rounded-full bg-white transition-transform ${
                          consciousnessAudio.isAudioEnabled ? 'translate-x-6' : 'translate-x-0.5'
                        }`}
                      />
                    </button>
                  </div>
