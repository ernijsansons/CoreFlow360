...
export const useConsciousness = (options: UseConsciousnessOptions = {}): UseConsciousnessReturn => {
      ...
      if (particle.awakened > 0.5) {
        ...
        for (const [otherId, otherParticle] of updatedParticles) {
          ...
          if (distance < connectionRadius) {
            ...
            if (
              !updatedConnections.has(connectionId) &&
              !updatedConnections.has(reverseConnectionId)
            ) {
              ...
              particle.intelligence = Math.min(
                1,
                particle.intelligence + 0.01 * intelligenceMultiplier
              )
              otherParticle.intelligence = Math.min(
                1,
                otherParticle.intelligence + 0.01 * intelligenceMultiplier
              )
              ...
              eventQueue.current.push({
                ...
                data: {
                  from: id,
                  to: otherId,
                  strength: updatedConnections.get(connectionId)!.strength,
                },
              })
            }
          }
  ...
  const metrics: ConsciousnessMetrics = {
    ...
    intelligenceLevel:
      Array.from(particles.values()).reduce((sum, p) => sum + p.intelligence, 0) / particles.size,
