    ...
    try {
      ...
      const summary: AuditSummary = {
        ...
        anonymizationSecurity:
          anonymizationResults.overallRisk === 'low'
            ? 90
            : anonymizationResults.overallRisk === 'medium'
              ? 70
              : anonymizationResults.overallRisk === 'high'
                ? 40
                : 20,
        dpiaCoverage:
          dpiaResults.assessment.residualRisk === 'low'
            ? 90
            : dpiaResults.assessment.residualRisk === 'medium'
              ? 70
              : dpiaResults.assessment.residualRisk === 'high'
                ? 40
                : 20,
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Comprehensive privacy compliance auditing system</p>
        </div>
        <Button onClick={runPrivacyAudit} disabled={isLoading} className="min-w-[140px]">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span
                        className={getRiskColor(
                          auditSummary!.consentCompliance < 70 ? 'high' : 'low'
                        )}
                      >
                    ...
                    <div className="flex justify-between">
                      ...
                      <span
                        className={getRiskColor(
                          auditSummary!.dataMinimization < 70 ? 'high' : 'low'
                        )}
                      >
                    ...
                    <div className="flex justify-between">
                      ...
                      <span
                        className={getRiskColor(
                          auditSummary!.anonymizationSecurity < 70 ? 'high' : 'low'
                        )}
                      >
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    High-priority privacy risks requiring immediate attention
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-2">
                    {auditResults.consent.mismatches.filter((m) => m.severity === 'critical')
                      .length > 0 && (
                      <Alert className="border-red-200">
                        <AlertDescription>
                          ...
                          critical consent violations detected
                        </AlertDescription>
                      </Alert>
                    )}
                    ...
                    {auditResults.portability.issues.filter((i) => i.severity === 'critical')
                      .length > 0 && (
                      <Alert className="border-red-200">
                        <AlertDescription>
              ...
              <CardContent>
                <div className="mb-4 grid grid-cols-3 gap-4">
                  ...
                  <div className="text-center">
                    ...
                    <div className="text-muted-foreground text-sm">Consented</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-bold">
                      {auditResults.consent.mismatches.length}
                    </div>
                    <div className="text-muted-foreground text-sm">Violations</div>
                  </div>
                </div>
                  ...
                  {auditResults.consent.mismatches.map((mismatch, index) => (
                    ...
                    >
                      <div>
                        ...
                        <div className="text-muted-foreground text-sm">User: {mismatch.userId}</div>
                      </div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="mb-4 grid grid-cols-3 gap-4">
                  <div className="text-center">
                    <div className="text-2xl font-bold">
                      {auditResults.minimization.totalFields}
                    </div>
                    <div className="text-muted-foreground text-sm">Data Fields</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-bold">
                      {auditResults.minimization.issues.length}
                    </div>
                    <div className="text-muted-foreground text-sm">Issues Found</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-bold">
                      ${Math.round(auditResults.minimization.potentialSavings.cost)}
                    </div>
          ...
          <TabsContent value="anonymization" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Re-identification attack simulation and risk assessment
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="mb-4">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge
                      className={`${getRiskColor(auditResults.anonymization.overallRisk)} border`}
                    >
                ...
                <div className="space-y-2">
                  ...
                  {auditResults.anonymization.attacks.map((attack, index) => (
                    <div key={index} className="rounded border p-3">
                      ...
                      <div className="text-muted-foreground mb-2 text-sm">{attack.description}</div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="mb-4 grid grid-cols-2 gap-4">
                  <div className="text-center">
                    <div className="text-2xl font-bold">
                      {auditResults.cookies.overallCompliance}%
                    </div>
                    <div className="text-muted-foreground text-sm">Compliance Score</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-bold">
                      {auditResults.cookies.avgChurnPrediction}%
                    </div>
                    <div className="text-muted-foreground text-sm">Predicted Churn</div>
                  </div>
                </div>
                    ...
                    <div key={index} className="rounded border p-3">
                      ...
                      {pattern.cookiesSetBeforeConsent.length > 1 && (
                        <div className="mt-1 text-xs text-red-600">
                          ⚠️ Non-essential cookies set before consent:{' '}
                          {pattern.cookiesSetBeforeConsent.join(', ')}
                        </div>
                      )}
                    </div>
                  ))}
            ...
            <Card>
              ...
              <CardContent>
                <div className="mb-4 grid grid-cols-2 gap-4">
                  <div className="text-center">
                    <div className="text-2xl font-bold">
                      {auditResults.portability.overallScore}%
                    </div>
                    <div className="text-muted-foreground text-sm">Portability Score</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-bold">
                      {auditResults.portability.issues.length}
                    </div>
                    <div className="text-muted-foreground text-sm">Export Issues</div>
                  </div>
                </div>
