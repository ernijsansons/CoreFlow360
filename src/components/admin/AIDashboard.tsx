...
export default function AIDashboard() {
  ...
  const [selectedView, setSelectedView] = useState<'overview' | 'models' | 'predictions' | 'training' | 'insights'>('overview')
  ...
  const handleOptimize = async () => {
    try {
      ...
      if (improvements.length > 0) {
        improvements.slice(0, 3).forEach((improvement: any, index: number) => {
          setTimeout(() => {
            toast.success(`${improvement.optimization}: ${improvement.improvement}`, { duration: 4000 })
          }, index * 1500)
        })
      }
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              <BrainIcon className="w-10 h-10 text-indigo-600 mr-4" />
              <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
              <div className="p-4 bg-gray-50 rounded-lg">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-2xl font-bold text-blue-600">{data.systemHealth.queueSize}</span>
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          >
            ...
            <p className="text-sm text-gray-600">
              {data.models.length} total models
            </p>
          </motion.div>
          ...
          >
            ...
            <p className="text-sm text-gray-600">
              {data.customModels.length} custom models
            </p>
          </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white rounded-xl shadow-lg p-6">
                ...
                <div className="space-y-4">
                      ...
                      <div key={model.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                        <div className="flex items-center">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-600">
                              {model.type} | v{model.version} | {model.performance.inferenceTime}ms avg
                            </p>
                          </div>
                        </div>
                        <div className="text-right">
                  ...
                  {data.customModels.length > 0 && (
                    <>
                      <h4 className="text-lg font-semibold text-gray-900 mt-6 mb-3">Custom Models</h4>
                      {data.customModels.map((model) => (
                        <div key={model.id} className="flex items-center justify-between p-4 bg-purple-50 rounded-lg">
                          <div className="flex items-center">
                            ...
                            <div>
                              <h4 className="font-semibold text-gray-900">{model.name}</h4>
                              <p className="text-sm text-gray-600">
                                {model.architecture} | Training Progress: {trainingProgress[model.id] || 0}%
                              </p>
                            </div>
                          </div>
                          <div className="text-right">
            ...
            {selectedView === 'predictions' && (
              <div className="bg-white rounded-xl shadow-lg p-6">
                <h3 className="text-xl font-semibold mb-4">Make Prediction</h3>
                <div className="space-y-4">
                  <div>
                    ...
                    >
                      ...
                      {data.models.filter(m => m.status === 'ready').map((model) => (
                  ...
                  {predictionResult && (
                    <div className="mt-6 p-4 bg-green-50 rounded-lg">
                      <h4 className="font-semibold text-green-900 mb-3">Prediction Result</h4>
                      <div className="space-y-2 text-sm">
                        <div><strong>Prediction:</strong> {JSON.stringify(predictionResult.prediction)}</div>
                        <div><strong>Confidence:</strong> {(predictionResult.confidence * 100).toFixed(1)}%</div>
                        <div><strong>Model Used:</strong> {predictionResult.modelUsed}</div>
                        <div><strong>Processing Time:</strong> {predictionResult.processingTime}ms</div>
                        {predictionResult.explanation && (
                          <div><strong>Explanation:</strong> {predictionResult.explanation}</div>
                        )}
                      </div>
                    </div>
                  )}
              ...
              <div className="bg-white rounded-xl shadow-lg p-6">
                ...
                <div className="space-y-4">
                  <div className="p-4 bg-blue-50 rounded-lg">
                    <h4 className="font-semibold text-blue-900 mb-2">Quick Train</h4>
                  ...
                  {data.insights.map((insight, index) => (
                    <div key={index} className="p-4 bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg">
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center">
                          <LightBulbIcon className="w-6 h-6 text-purple-600 mr-3" />
                          <span className={`inline-flex px-2 py-1 rounded text-xs font-medium ${
                            insight.type === 'performance' ? 'bg-blue-100 text-blue-700' :
                            insight.type === 'accuracy' ? 'bg-green-100 text-green-700' :
