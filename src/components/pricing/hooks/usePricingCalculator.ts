...
export const usePricingCalculator = (
  ...
  const [billingPeriod, setBillingPeriodState] = useState<'monthly' | 'annual'>(
    defaultBillingPeriod
  )

  // Session tracking for analytics
  const [sessionId] = useState(
    () => `pricing_session_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
  )
  ...
  const isDepartmentSelected = useCallback(
    (departmentId: string) => selectedDepartmentIds.has(departmentId),
  ...
  const getRecommendedDepartments = useCallback((): string[] => {
    ...
    if (
      (selected.includes('sales') || selected.includes('marketing')) &&
